...
### Added
- **Eval: `dataset()` now flags annotation keys dropped by the schema.** Zod object schemas strip unknown keys by default, so an annotation field absent from the `annotations` schema was silently dropped before reaching scorers — a scorer reading that field saw `undefined` and quietly degraded to a no-op while the eval still "passed". `dataset()` now diffs each item's raw annotations against the parsed result (recursively, by key path) and, by default, emits a single consolidated `console.warn` per dataset listing the dropped paths (e.g. `annotation key(s) dropped by schema: expectedTone, persona.role`). New `onExtraAnnotationKeys?: 'warn' | 'error' | 'ignore'` on `DatasetConfig` (default `'warn'`) escalates to a thrown error (the strict path for CI) or restores the prior silent-strip behavior. Detection is mode-independent (works whether the schema strips, is `.strict()`, or is `.loose()`) and source-independent — critically, it covers **file-based datasets**, where the JSON is never type-checked and TypeScript's excess-property check can't help. Scoped to `annotations` only; `input` strips are left untouched because they flow to the workflow where they surface. Exported `ExtraKeyPolicy` type. Non-breaking — a warning, not a throw.
- **Eval + Studio: dropped annotation keys surface in the Eval Runner UI.** The dataset exposes the most recent run's dropped keys via `Dataset.droppedAnnotationKeys` (populated by `getItems()`), which `runEval()` folds into `EvalResult.metadata.droppedAnnotationKeys` (present only when non-empty). It flows untouched through Studio's redaction and REST layers, and the Eval Runner panel renders an amber "_N annotation keys dropped by the dataset schema_" banner above the run's stat cards, chipping each dropped key path — so the signal isn't limited to the CLI's stderr. `getResultDroppedAnnotationKeys()` client helper added.
