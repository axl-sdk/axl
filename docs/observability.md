...
A `maxContext` summary call shows up under its ask as its own
`agent_call_start` / `agent_call_end` pair with `data.purpose: 'summary'`,
`turn: 0`, and the summary model's URI. Its cost counts once toward the ask and
budget; an unpriced summary marks both as lower bounds. Reusing a remembered
summary makes no call and emits no pair.
...
- **`ctx.events` does not bridge process loss at `ctx.awaitHuman()`.** The continuation and bus belong to the current runtime. A persisted orphan request remains visible but Axl does not replay it; use an application-owned durable approval protocol (see Security > Approval Gates). Until request-scoped decision IDs ship, two concurrent `awaitHuman()` calls in one execution fail loudly with `CONCURRENT_HUMAN_DECISION_UNSUPPORTED` instead of overwriting a resolver.
- **Approval cleanup failures are control-plane signals, not trace events.** If cancellation compensation cannot remove a persisted decision, the row remains visible and `runtime.on('decision_cleanup_failed', ...)` receives `{ executionId, workflow?, operation, error }`. Alert from this trusted-host event and retry cleanup or call `deleteExecution`; it is intentionally excluded from `AxlEvent` persistence and Studio trace rendering.
...
- **Check `.observationStatus`.** After consuming an `AxlEventBus` or `AxlStream`, inspect this property before asserting complete start/end pairing. It aggregates loss across the main iterator and bounded `stringStream()` subscribers. Either overflow policy records `{ complete: false, reason: 'queue_overflow', droppedEvents }`; strict mode then also throws.
...
- `reasoning_context_reset` fires once per affected provider call, including
  repeated calls in one tool loop, and once per ask with
  `reasons: { client_prefix_rewrite: n }` before the first `agent_call_start`
  when a `maxContext` summary removed Anthropic thinking itself. Session-level
  trimming (`history.maxMessages`) happens before the execution and is not
  reported. Adapter responses and terminal stream chunks carry the same counts
  in `diagnostics.reasoningContextReset`. Only fixed numeric counts reach
  events, even from custom providers; signed blocks and raw bodies never do.
  Studio shows the count and reason categories, and labels mixed
...
`observation: { complete: false, reason: 'process_interrupted' }`, and
