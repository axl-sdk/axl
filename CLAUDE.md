...
- Eval enriched results: `EvalItem` has `duration`, `cost`, `scorerCost`, `scoreDetails` (per-scorer `ScorerDetail` with metadata/timing/cost), `metadata?` (execution context forwarded from runtime: `models`, `tokens`, `agentCalls`). `EvalSummary` has `timing` stats. `EvalComparison` has `timing`/`cost` deltas, `ci?`/`significant?` per scorer. `EvalRegression`/`EvalImprovement` include `itemIndex`. Runner pre-allocates `evalItems` array for deterministic item ordering under concurrency. `runEval()` aggregates unique models from item metadata into `EvalResult.metadata.models`
...
- `runtime.trackExecution(fn)` wraps an async function with `AsyncLocalStorage`-based cost and metadata attribution; returns `{ result, cost, metadata }` where metadata includes `models` (unique URIs), `tokens` (input/output/reasoning sums), and `agentCalls` count. `trackCost(fn)` is a convenience wrapper returning `{ result, cost }`. Both used by eval runner and CLI for per-item scoping
