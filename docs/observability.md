...
**`parentToolCallId` on nested events.** When an agent-as-tool handler spawns a child `WorkflowContext` (via `ctx.createChildContext()` inside a tool), every event emitted from the child stamps `parentToolCallId: string` — the `callId` of the outer `tool_call_end`. Consumers can join `event.parentToolCallId === toolCall.id` to reconstruct call graphs. This field is `@deprecated` going forward — use `parentAskId` (on `AskScoped`, see below) for the same correlation in the unified model. The Trace Explorer visualizes nesting via `getDepth()`.
