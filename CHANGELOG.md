...
### Added
- **Typed `ProviderError` with retryable classification + network normalization.** Provider adapters now throw a `ProviderError` (extends `AxlError`, `code: 'PROVIDER_ERROR'`) at every non-2xx HTTP site instead of a bare `Error`, and `fetchWithRetry` normalizes a thrown network failure (DNS, reset, TLS, socket hangup) into a `ProviderError` with `status: 0` after exhausting retries — a user/budget abort still propagates verbatim. The error carries `.provider`, `.status`, `.retryable`, `.retryAfterMs?` (raw/unclamped, numeric-seconds **and** HTTP-date), `.requestId?`, and `.body?` (raw provider body — kept off the event stream as redaction-eligible content). The `.message` is the provider's text **verbatim** (no prefix), so existing message assertions are unaffected — only the thrown *type* changed. `ProviderError` and the `isRetryableStatus(status)` classifier are exported from `@axlsdk/axl`. `ProviderError.retryable` is a **broader semantic failover hint** (`408`/`429`/`5xx`/`529`/network ⇒ retryable; `4xx` incl. `404`/`409` ⇒ not) and is **deliberately separate** from the unchanged narrow transport auto-retry set (`429`/`503`/`529`). The in-loop transport sleep now clamps `Retry-After` to 60s (the raw value still rides on the error). `agent_call_end` error events additionally carry `data.status` and `data.retryable` when the error was a `ProviderError`. Purely additive.
- **Budget honesty for unpriced models.** `ctx.budget()` now reports when its total is a lower bound: `BudgetResult` gains `unpriced: boolean` and `ctx.getBudgetStatus()` returns `{ …, unpriced }` (true when a call did measurable work but had no usable cost). Inner budgets propagate the flag to the parent, and Axl logs a one-time `console.warn` per block. **This is reporting, not enforcement** — the cost limit / `hard_stop` still cannot trip on unpriced spend (the enforcement rail never sees the unknown component), so a `hard_stop` budget does not govern unpriced/self-hosted/Bedrock models. Token-denominated budgets are the planned fix. See `docs/observability.md#budget-honesty`.
- **Honest aggregate cost for unpriced models.** Tier 1 made per-call cost honest (`undefined`, not a fake `$0`); the per-ask rollup now is too. `ask_end` carries `unpriced?: boolean` — set when a cost-bearing call did measurable work (returned usage/tokens) but had no usable cost (unpriced model / pricing-table miss). When set, `ask_end.cost` is a **lower bound** (a failed call, which has no usage, is NOT flagged). Studio surfaces it: the Cost Dashboard shows `≥ $X` with an "N unpriced calls" note (new `CostData.unpricedCalls`), and the trace explorer's cost badges render `≥ $X` with a tooltip. `agent_call_end.cost` is now `number | undefined` (it always emitted the optional `response.cost`). The lower-bound signal is also surfaced on the **aggregate** cost surfaces a user reads directly — `ExecutionInfo.unpriced` (from `runtime.execute()` / `getExecutions()` / recovered streams), `runtime.trackExecution().unpriced`, and `AxlTestRuntime.unpriced()` — so callers no longer have to scan the event timeline to know whether `totalCost` is exact. A single exported `isUnpricedLeaf(event)` helper is now the one source of truth for the discriminator (the core ask rollup, the runtime aggregation, and Studio's count all route through it).
- **Async API-key callback (expiring credentials).** A provider's `apiKey` (and every provider constructor) now accepts `string | (() => string | Promise<string>)` — a function is resolved **per request** so the caller's callback owns token refresh (Azure-Entra, Databricks/IBM OAuth, AWS Bedrock short-term tokens). Resolved before the rate-limiter permit; a plain string is unchanged. Exported `ApiKeySource` + `resolveApiKey`. Additive (a string stays assignable); a `*.test-d.ts` guard locks it in.
- **AWS Bedrock preset (`bedrock:`).** gpt-oss via Bedrock's OpenAI-compatible endpoint with bearer auth (`AWS_BEARER_TOKEN_BEDROCK`, no SigV4). Region-specific base URL (prefer the `bedrock-mantle` endpoint; `requireExplicitBaseUrl`), versioned model ids (`bedrock:openai.gpt-oss-120b`), `reasoning_effort` for gpt-oss, cost `unknown` (Bedrock returns no per-call cost). Claude-on-Bedrock is deferred to a later native-adapter mode.
