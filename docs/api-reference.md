...
| `.observationStatus` | `ObservationStatus` | Machine-readable completeness across this bus's bounded iterable views, including each `stringStream()` subscriber. `{ complete: false, reason: 'queue_overflow', droppedEvents }` counts dropped deliveries under either overflow policy; `{ complete: true }` otherwise. EventEmitter listener delivery is independent |
...
persisted for operator visibility, but the continuation is owned by the current
runtime process. A request left after process loss cannot be resolved through
Axl; use an application-owned durable command/idempotency protocol for that
case (see Security > Approval Gates).
...
and denial may carry only optional string `reason`. Arrays, accessors, symbol or
unknown keys (including non-enumerable keys), truthy non-booleans, and
contradictory branch fields fail with `AxlError` code
...
same explicit execution scope evaluates the checkpoint again. Axl does not
automatically restart or resume workflows across processes; do not use a
checkpoint alone as an exactly-once side-effect boundary.
...
- **Ask-level** (controlled by `AgentConfig.maxContext`). Triggers inside `ctx.ask()` when the prompt + history would exceed the agent's configured context window. Its summary is an execution-local projection of an exact history prefix; it is never saved as session metadata or used to trim another agent's history. Regeneration includes the durable session summary and every newly covered message. A cache may be reused within the execution while its covered prefix is unchanged and its complete tail fits. Separate `Session.send()` executions may each require a metered summary call.
...
| `stream.observationStatus` | `ObservationStatus` | Machine-readable completeness across the stream's bounded iterable views; reports `queue_overflow` and the number of dropped deliveries under either overflow policy, including `stringStream()` subscriber loss |
...
| `registerTool(...tools)` | Register one or more standalone tools. Compatible ESM/CJS copies can register and invoke each other's `tool()` definitions, including model-requested calls |
...
| `resolveDecision(executionId, decision)` | `Promise<void>` | Resolve a pending human decision in the owning runtime. The resolver is published before `await_human`, store cleanup completes before the workflow is released, and concurrent resolution or cancellation shares one store-mutation claim. Unknown/already-claimed IDs fail with `PENDING_DECISION_NOT_FOUND`; persisted requests with no in-process owner fail with `CROSS_PROCESS_RESUME_UNSUPPORTED` before mutation |

`ExecuteOptions` (accepted by both `execute()` and `stream()`; exported from `@axlsdk/axl`):

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `metadata` | `Record<string, unknown>` | `{}` | Metadata passed to the workflow context. Reserved keys: `sessionId`, `sessionHistory` |
...
| `branchDrainTimeoutMs` | `number` | `5_000` | Maximum terminal wait for abort-ignoring `race` / quorum branch continuations. On expiry, finalization proceeds and `workflow_end.data.observation` plus `ExecutionInfo.observation` report `branch_drain_timeout` |
...
| `defaultTtl` | `number?` | — | Default TTL in **seconds** applied to every TTL-capable storage category. Without a TTL, every `save*` accumulates forever — strongly recommended in production |
| `ttls` | `object?` | — | Per-category TTL overrides in seconds (`session`, `sessionMeta`, `checkpoint`, `executionState`, `executionHistory`, `evalHistory`, `memory`, `streamingEvents`). Set a category to `null` to explicitly opt out even when `defaultTtl` is set. `pendingDecision` is intentionally NOT configurable — pending decisions share a hash, so a TTL would expire all of them at once. **Window semantics:** `memory`, `session`, and `sessionMeta` are sliding (every write refreshes TTL — active users keep their data, inactive users forget; reads do NOT refresh); `checkpoint` and `streamingEvents` are fixed-creation (TTL set on first write only); `executionState` / `executionHistory` / `evalHistory` are fixed-refresh (TTL via `SET ... EX`). **`streamingEvents` is opt-in only** — does NOT fall back to `defaultTtl`. Must be longer than your max time-between-restarts or `recoverIncompleteStreams()` will miss crashed-run buffers that aged out before the next process started. Non-positive and non-finite values (`0`, negative, `NaN`, `Infinity`) are defensively treated as no-TTL. Per-category TTLs are independent. Legacy memory entries (pre-`memory-methods` patch) migrate forward on first read with the configured TTL applied. `listPendingExecutions` self-prunes stale ids when their `executionState` blob has TTL-evicted |

### Provider configuration

`config.providers.<name>` (`ProviderConfig`) accepts:
...
valid, so treat every field as possibly absent.
...
**Checkpoints** (named result storage within an execution scope):
...
**Legacy execution-state storage** (retained for custom-store compatibility; the runtime does not automatically resume it):

| Method | Description |
|--------|-------------|
| `saveExecutionState(executionId, state)` | Save an application-managed execution-state record |
...
| `AxlError` / `CONCURRENT_HUMAN_DECISION_UNSUPPORTED` | `ctx.awaitHuman()` | A second execution-keyed wait would overwrite the first. Both waits are cancelled; request-scoped concurrent approvals are deferred to the durable replay design |
...
breakdown would blame tools and gates for a budget the provider simply never measured.
...
| `onExtraAnnotationKeys` | `'warn' \| 'error' \| 'ignore'` | `'warn'` | What to do when an item's `annotations` contain keys not declared in the `annotations` schema. Zod strips unknown keys by default, so such fields are silently dropped before reaching scorers (a common cause of no-op scorers). `'warn'` logs one consolidated `console.warn` per dataset listing the dropped key paths; `'error'` throws; `'ignore'` restores the silent-strip behavior. Detection is recursive (reports nested paths like `persona.role`) and covers file-based datasets, which get no compile-time checking. Only runs when the `annotations` schema is a `ZodObject` — schemas wrapping a `.transform()`/`.preprocess()` are skipped, since their parsed output is reshaped by design. Scoped to `annotations`, not `input`. |

> **Annotation/schema drift.** A scorer that reads `annotations?.someKey` silently returns garbage when `someKey` was stripped because it isn't in the `annotations` schema (e.g. a typo, or a key added to a `file:` dataset but not the schema). On Zod v4 TypeScript catches *some* of this — an inline item literal triggers an excess-property error, and a schema declared `z.ZodType<T>` that is structurally narrower than `T` is itself a compile error. What stays uncaught at compile time is variable-sourced items, `file:` (JSON) datasets, and explicit `as unknown as` casts. `onExtraAnnotationKeys` (default `'warn'`) is the runtime backstop that catches all of those regardless. Set it to `'error'` in CI to fail the build on drift.
