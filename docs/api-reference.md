...
| `ttls` | `object?` | — | Per-category TTL overrides in seconds (`session`, `sessionMeta`, `checkpoint`, `executionState`, `executionHistory`, `evalHistory`, `memory`). Set a category to `null` to explicitly opt out even when `defaultTtl` is set. `pendingDecision` is intentionally NOT configurable — pending decisions share a hash, so a TTL would expire all of them at once. **Window semantics:** `memory` and `session` are sliding (every write refreshes TTL — active users keep their data, inactive users forget); `checkpoint` and `sessionMeta` are fixed-creation (TTL set on first write only); `executionState` / `executionHistory` / `evalHistory` are fixed-refresh (TTL via `SET ... EX`). Non-positive and non-finite values (`0`, negative, `NaN`, `Infinity`) are defensively treated as no-TTL. Per-category TTLs are independent — `session: 3600` with `sessionMeta: 86400` leaves orphaned metadata until the metadata TTL fires |
