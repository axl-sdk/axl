 ...
 *   (alternative, SigV4-capable runtime: `https://bedrock-runtime.{region}.amazonaws.com/openai/v1`).
 * - Auth: a bearer API key (`AWS_BEARER_TOKEN_BEDROCK`); short-term tokens can use
 *   the async `apiKey` callback (T2.2). No SigV4.
 * - Model ids are AWS-namespaced. The **mantle** endpoint uses UNSUFFIXED ids
 *   (`bedrock:openai.gpt-oss-120b`, `bedrock:openai.gpt-oss-20b`); the runtime
 *   endpoint uses the version-suffixed form (`openai.gpt-oss-120b-1:0`). Match the
 *   id to your endpoint. (The registry splits the URI on the first colon, so a
 *   suffixed id round-trips fine.)
