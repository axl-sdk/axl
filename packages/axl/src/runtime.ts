...
export class AxlRuntime extends EventEmitter {
  ...
  async trackExecution<T>(
     ...
     *
     * `calls` counts the TIMED calls only — it can be lower than the same
     * model's `modelCallCounts` entry when a provider omits `timing` or a call
     * failed (the error path carries no timing). Divide a sum by `calls`, never
     * by `modelCallCounts`.
     *
    ...
    const listener = (event: AxlEvent) => {
      ...
      if (event.type === 'agent_call_end') {
        ...
        if (event.model && event.timing) {
          const t = event.timing;
          let bucket = modelTiming.get(event.model);
          if (!bucket) {
            bucket = { calls: 0, queuedMs: 0, retryMs: 0, wireMs: 0 };
