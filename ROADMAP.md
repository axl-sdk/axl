...
- **Per-Call Latency Observability** — `CallTiming` on every built-in adapter's response and streamed `done` chunk, copied to `agent_call_end.timing`: the SDK's own rate-limiter queue, provider retry backoff, time-to-headers, first token, and provider wire time as separate buckets beside the unchanged `duration`. `TimeoutError` explains where a timed-out ask's budget went; `axl-eval` reports call-weighted per-model latency separate from wall clock. See [observability.md#per-call-timing](docs/observability.md#per-call-timing).
- **Unified Effort** — Cross-provider `effort` uses exact model and endpoint capabilities, including native `max` on GPT-5.6 Responses and Claude 5 (with GPT-5.6 Chat capped to `xhigh`), adaptive thinking on supported Claude families, `thinkingLevel` on Gemini 3.x, and `thinkingBudget` on Gemini 2.x. Every clamp an adapter applies is reported once per ask as a `provider_diagnostic` event (`effort_clamped`) via the optional `Provider.effortResolution()` capability. See [observability.md#provider-diagnostics](docs/observability.md#provider-diagnostics).
- **Current Provider Catalog and Honest Pricing** — GPT-6, Claude Opus 5.5, Gemini 3.8 Flash, and current compatible-provider profiles ship with exact model matching, observable cache-write accounting, context/tier-aware pricing, and fail-closed unknown cost. New exact IDs still require live provider certification; defaults remain unchanged pending paired evals of task success, latency, and cost per success.
