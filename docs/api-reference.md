...
| `string_delta` | `AskScoped` | `attempt: number`, `data: StringDeltaData` (`{ path: string, delta: string }`) | Per-chunk character-level deltas inside string VALUES of progressive structured output. Same gating as `partial_object` (schema set, no tools, root is `ZodObject`). `path` is an RFC 6901 JSON Pointer to the string field (`/summary`, `/sources/0/title`); `delta` is the unescaped chars added in this chunk. Designed for chat-style typewriter rendering of long string fields — see `AxlStream.stringStream` / `AxlEventBus.stringStream` view helpers below |
