...
- **`importModule`: try the dynamic import before attempting tsx registration.** `axl-eval` and `axl-studio` use `importModule` (in `@axlsdk/axl`'s shared `cli-internals`) to load user `.ts`/`.mts` config files. Previously it preemptively called `ensureTsxRegistered()` for every TS file, which does `await import('tsx/esm/api')` — and tsx isn't resolvable from the axl package's own `node_modules` under pnpm's strict isolation. The result: when the parent process had already activated tsx (e.g., `tsx watch src/cli.ts` for `pnpm dev:studio`), `importModule` still failed with "tsx is not installed" because the registration step itself couldn't load tsx. Fix: try the dynamic import first; only attempt registration when Node surfaces a recognizable missing-loader error (`Unknown file extension` / `Cannot use import statement outside a module`). The actionable "tsx is not installed" error message is preserved for the genuine no-loader-anywhere case. Caught while running `pnpm dev:studio` in this monorepo.
...
- **Sessions: documented sharing semantics, the "turn" definition, and concurrency caveats.** The Sessions section in `docs/api-reference.md` previously listed only options and methods. Added four subsections: "What's stored" (flat `ChatMessage[]` keyed by `sessionId`, the `Session` object holds no message cache, lazy reads), "Sharing semantics" (history is session-scoped, not agent- or workflow-scoped — multi-workflow and multi-agent calls share one history with no attribution; child contexts isolate; failed retries are stripped), "Turns vs. exchanges" (a "turn" in axl is one provider call inside `ctx.ask()`, not a user↔assistant round-trip), "Concurrency and races" (read-modify-write on `send()` has no locking; concurrent calls on the same id race), and "Summarization" (session-level vs ask-level paths are independent and can both fire in one `send()`). Also annotated `AgentConfig.maxTurns` with a pointer to the Turns vs. Exchanges note. No code change. Reported by a user who couldn't tell from the docs whether sessions partitioned by workflow.
