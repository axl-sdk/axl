...
### Added
- **Conditional scorers (`applies`).** `scorer()` and `llmScorer()` accept an optional `applies?: (output, input, annotations?) => boolean` predicate that scopes a scorer to a subset of items (a refusal judge for refusal-expected items, a constraint judge for constrained items, …). When it returns `false` the scorer is skipped: the body never runs — for an `llmScorer`, **no provider call is made** — and the item counts as neither `scored` nor `failed`, so it's excluded from the mean *and* from the failure-rate denominator (`failOnScorerErrorRate` and `compare --max-scorer-error-rate`). The skip is visible everywhere a score is: `ScorerDetail.skipped`, a per-scorer `EvalSummary.scorers[].skipped` count (summed across runs by `aggregateRuns`), `EvalComparison.scorers[].{baseline,candidate}Skipped`, the CLI table (`(N N/A)`), and Studio's Eval Runner (a neutral "N/A" chip on the summary, multi-run, and per-item views). A predicate that throws is recorded as a scorer failure, not a skip. Two new non-fatal CLI advisories close silent-mislead holes: a `NOTE` when a scorer ends up applicable to zero items (its mean is meaningless), and a `NOTE` in `compare` when the two sides skipped different counts (the means cover different subsets). In Studio's compare view, a per-scorer "paired n" note now makes explicit that the headline `delta` is the difference of two independent per-side means while the CI is paired over the items scored on both sides — shown only when those samples diverge. `EvalComparison.scorers[].n` (the paired sample size) is now always populated, not only when a CI is computed. `ScorerApplies` is exported. Purely additive — back-compatible.
