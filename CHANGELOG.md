...
### Added
- **Eval: an item's scorers now run concurrently.** Previously each item's scorers ran serially (`for (const s of scorers) await s.score(...)`), so raising `concurrency` parallelized *across* items but never the judge phase *within* an item — the dominant cost for evals with many `llmScorer` judges. A new `scorerConcurrency?: number` on `EvalConfig` (and `RescoreOptions`) bounds the per-item scorer fan-out; **default 5**, matching the item-level `concurrency` default. Worst-case concurrent scorer calls is therefore `concurrency × scorerConcurrency` (≤25 at defaults), softened by the provider's 429/503/529 backoff. Cost, per-scorer timing, and `scores`/`scoreDetails`/`scorerErrors` ordering are all preserved deterministically (results are folded back in scorer-array order regardless of completion order). Shared `mapWithConcurrency` worker-pool helper now backs both the item loop and the scorer loop in `runEval`/`rescore`. **See _Changed_ for the behavior-change note.**
...
- **Eval CLI: `--scorers <a,b>` filter.** Run only the named scorers for a focused iteration loop (single eval file only; unknown names error with the available list). The result is stamped `metadata.scorerFiltered: true` + `metadata.scorersRun: string[]` so a subset run can't be silently used as a full baseline — `axl-eval compare` prints a warning when either side is filtered, and **refuses to gate** (`--fail-on-regression` exits non-zero) on a filtered result. Distinct from the multi-run "partial" concept.
