...
- **`ctx.events` and `awaitHuman` continuations are process-local.** A persisted pending request remains visible after process loss, but Axl does not replay the workflow. Use an application-owned durable approval protocol; see the approval-gate security notes.
