...
- **Anti-anchoring (load-bearing):** pin the scope, then have an **independent
  subagent** derive the scenario list without the diff, changed-file list,
  implementation summary, or suspected gaps. The root may already know the
  implementation; the blind subagent is the isolation boundary. Freeze its
  scenario list before anyone maps scenarios to code.
...
- Route provider-gated scenarios into the plan's single live-API verification
  checklist, or one review-local checklist when no plan exists, and close them
