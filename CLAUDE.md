...
- AxlStream wire format: pure `AxlEvent` (no per-stream synthesized shape). The translation layer that derived legacy `StreamEvent` shapes (`agent_end`, `tool_result`, `step`) was deleted in 0.16.0; runtime is a pure fan-out from `emitEvent` to the wire. `AxlStream.text` filters root-only tokens (depth=0). `AxlStream.lifecycle` (renamed from `.steps`) filters to structural events: `ask_*`, `agent_call_*`, `tool_call_*`, `tool_approval`, `tool_denied`, `handoff`, `delegate`, `pipeline`, `verify`, `workflow_*`. `AxlStream.fullText` excludes nested-ask tokens (consumers wanting nested can iterate the stream and filter on `event.depth >= 1`). `STREAM_EVENTS` set derived from `AXL_EVENT_TYPES` so adding a new variant auto-extends subscribable events
