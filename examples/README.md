...
These examples use the checked-out `packages/axl` package. Build it before
installing the examples so its local `dist/` exists:
...
| [`image-lighthouse.ts`](./image-lighthouse.ts) | `IMAGE_MODEL=openai-responses:gpt-4o-mini npm run image-lighthouse` | Ordered image input, structured visual finding, and a second-agent verification. |
