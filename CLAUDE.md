...
import type { ScorerResult, ScorerDetail, EvalCompareOptions, RescoreOptions, MultiRunSummary, BootstrapCIResult, EvalProgressEvent, RunEvalOptions } from '@axlsdk/eval';
...
packages/axl-eval/src/
  ...
  runner.ts          — runEval() with concurrent execution, onProgress callback, signal cancellation
...
packages/axl-studio/src/
  ...
  client/
    ...
    panels/
      ...
      eval-runner/   — Eval execution + comparison (EvalSummaryTable, EvalItemList, EvalItemSidebar, EvalItemDetail, ScoreDistribution, EvalCompareView, EvalHistoryTable, EvalCompareItemTable, EvalCompareRunPicker, EvalMultiRunSwitcher, EvalCommandBar, eval-store). Score colors: 3-tier system (>=0.8 green, >=0.5 amber, <0.5 red). eval-store.ts is a module-level external store (useSyncExternalStore) that survives route changes — tracks active streaming eval execution state (progress, result, error)
```
...
- `TraceEvent` is a **discriminated union** over `type` — consumers that narrow via `type === 'agent_call'` etc. get statically-typed access to per-variant `data` and variant-specific required fields (e.g. `tool: string` is required on `tool_call`/`tool_approval`/`tool_denied`). Per-type data shapes are exported: `AgentCallTraceData`, `GuardrailTraceData`, `SchemaCheckTraceData`, `ValidateTraceData`, `ToolApprovalTraceData`, `ToolCallTraceData`, `HandoffTraceData`, `DelegateTraceData`, `VerifyTraceData`. When adding a new trace type, extend the union in `types.ts` AND `WorkflowContext.emitTrace()` together — the internal emitter uses a loose partial type with a single localized `as unknown as TraceEvent` cast to keep call-site ergonomics, but the external type is strict
...
- Redaction: `config.trace.redact` is an **observability-boundary filter**, not a data-at-rest transform. Scrubs user/LLM content at three layers: (a) trace events at `emitTrace()` emission — `prompt`/`response`/`system`/`thinking`/`messages` on `agent_call`; `reason`/`feedbackMessage` on gate events; `args`/`result` on `tool_call`; `args`/`reason` on `tool_approval`; `message` on `handoff` (roundtrip); string fields on `log` events with one-level walk that preserves nested numeric fields (`memory_*.data.usage.tokens/.cost` survive, `.model` scrubbed); `workflow_start.data.input`, `workflow_end.data.result/.error`; `agent_call.data.messages` → single-entry `ChatMessage[]` placeholder. (b) Studio REST routes at serialization — `ExecutionInfo.result/.error`, memory values (keys preserved), session `ChatMessage.content`/`tool_calls[*].function.arguments`/`providerMetadata` (roles/names preserved), `EvalResult.items[*].input/output/error/annotations/scoreDetails[*].metadata` (scores/duration/cost/summary/metadata preserved), `PendingDecision.prompt/metadata`, tool test results, workflow execute sync results. (c) Studio WS stream broadcasts — `StreamEvent` scrubbing via `redactStreamEvent`: `token.data`, `tool_call.args`, `tool_result.result`, `tool_approval.args/.reason`, `done.data`, `error.message`; `step.data` (already-scrubbed TraceEvent) passes through. Top-level numeric fields (`cost`, `tokens`, `duration`) are NEVER scrubbed — load-bearing for `trackExecution` and `CostAggregator`. Programmatic callers of `runtime.execute()` / `getStateStore()` still receive raw data; write endpoints (`PUT /api/memory`, `POST /api/sessions/:id/send`) still accept raw data. Access the flag via `runtime.isRedactEnabled(): boolean` (narrow public getter added to avoid exposing the shallow-mutable full config). Helpers live in `packages/axl-studio/src/server/redact.ts`: `redactExecutionInfo`/`redactExecutionList`, `redactMemoryValue`/`redactMemoryList`, `redactSessionHistory`, `redactEvalResult`/`redactEvalHistoryList`, `redactPendingDecision`/`redactPendingDecisionList`, `redactStreamEvent`, and generic `redactValue`
...
- Nested call correlation: `createChildContext(parentToolCallId)` stamps the `callId` on every trace event emitted from the child. Consumers can join `event.parentToolCallId === toolCall.id` to reconstruct agent-as-tool call graphs. The tool execution path passes `toolCall.id` automatically
...
- Studio cost breakdown: `CostData.retry: {primary, schema, validate, guardrail, retryCalls}` decomposes `agent_call` cost by `retryReason`. Surfaces in the Cost Dashboard "Retry Overhead" section (only when retries > 0). Data flows automatically from `emitTrace` via the cost-aggregator's `retryReason` bucketing
- `ctx.verify()` emits a `verify` trace event once per terminal outcome with `{ attempts, passed, lastError? }`. Fires on success, on retry exhaustion (throws), and on fallback path
...
- `AxlRuntime.getEvalHistory()` returns eval run history (most recent first); `saveEvalResult(entry)` persists to in-memory cache + StateStore. `runRegisteredEval(name, options?)` auto-saves results; `options` accepts `{ metadata?: Record<string, unknown> }` to inject custom metadata
...
- `runEval()` stores `scorerTypes: Record<string, 'llm' | 'deterministic'>` in `EvalResult.metadata`. Accepts optional 4th arg `RunEvalOptions` with `onProgress` callback (`EvalProgressEvent` fired after each item completes) and `signal` (`AbortSignal` — checked before each item, marks remaining items as cancelled)
- `runtime.runRegisteredEval()` accepts `onProgress` and `signal` options, forwarded to `runEval()`
...
- Studio: `POST /api/evals/:name/run` accepts `{ runs: N, stream?: true }` body. Multi-run capped at 25. When `stream: true`, returns `{ evalRunId }` immediately and broadcasts progress events (`item_done`, `run_done`, `done`, `error`) over `eval:{evalRunId}` WS channel. When `stream` is absent/false, blocks and returns the full result (backward compatible). `POST /api/evals/runs/:evalRunId/cancel` aborts an active streaming run. When `runs > 1`, the final result includes `_multiRun: { aggregate: MultiRunSummary, allRuns: EvalResult[] }` enrichment
