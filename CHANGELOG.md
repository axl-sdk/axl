...
- Graceful `ctx.ask()` `timeout` now unconditionally excludes observed SDK governor wait from that ask's completed provider turns, alongside the existing `awaitHuman` exclusion. Parent tool time and sibling ask clocks remain independent; `signal`, `stallTimeout`, and `acquireTimeoutMs` retain their separate hard-deadline, request-stall, and admission-wait roles. `TimeoutError.breakdown` adds required `chargedMs`, and the message's diagnostic suffix appends `charged Nms` after the existing fields, which keep their meanings. The `axl-eval` failure-causes line groups graceful timeouts as `timeout` instead of `other`. Failed eval items preserve finite numeric timeout evidence, and Studio keeps that evidence under redaction. Because the exported `TimeoutBreakdown` shape now has a required field, callers constructing it must update; ship this source-breaking type change in the next 0.x minor release.
