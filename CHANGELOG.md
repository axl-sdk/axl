...
- **`maxConnections` limit** on `ConnectionManager`: Rejects new connections beyond 100 (configurable)
...
- **`readOnly` option** on `createServer()`: Disables all mutating API endpoints (returns 405)
