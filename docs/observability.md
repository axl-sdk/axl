...
- **`ctx.events` does NOT bridge across re-execution after `ctx.awaitHuman()` suspension.** A later execution gets a fresh `WorkflowContext` with its own bus. An observer holding the original `ctx.events` reference sees nothing from it. Use `runtime.on('trace', …)` for cross-execution observation, and note that the legacy cross-process resume helper is not an exactly-once decision protocol (see Security > Approval Gates).
- **`ctx.race()` losers emit complete event trails before workflow finalization.** The primitive returns its winner immediately to workflow code and aborts losers. The runtime then drains race/spawn-quorum/map-quorum branch continuations before `workflow_end` and persistence, so cancellation terminals and late measurable provider cost remain canonical. A provider or handler that ignores abort forever can therefore prevent runtime completion; branch code must be abort-aware. Consumers reconstructing "what won" must filter on the resolved value/winner, not on event presence.
