...
| `TimeoutError` | `ctx.ask()` | Graceful between-turn `timeout` expired. The message names the agent. When at least one completed turn reported provider `timing`, its existing suffix reports `elapsed`, `queued`, `retries`, `wire`, and `other`; `.breakdown` (`TimeoutBreakdown`) carries those fields plus required `chargedMs`. With no instrumented completed turn, the message is the bare prefix plus agent name and `.breakdown` is `undefined`. |
