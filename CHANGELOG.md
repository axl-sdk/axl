...
- Graceful `ctx.ask()` `timeout` now unconditionally excludes SDK governor wait observed in `fetchWithRetry` (first permit, spacing, adaptive pacing, 429 brake, re-acquire), recorded on every enclosing ask exactly like `awaitHuman`. A parent is credited while a nested ask in its tool queues; sibling ask clocks remain independent; reported `CallTiming.queuedMs` is diagnostic only and never earns credit; `signal`, `stallTimeout`, and `acquireTimeoutMs` retain their separate hard-deadline, request-stall, and admission-wait roles. `TimeoutError.breakdown` adds required `chargedMs` and `elapsedMs` is now plain wall time including paused waits, and the message's diagnostic suffix appends `charged Nms` after the existing fields, which keep their meanings. The `axl-eval` failure-causes line groups graceful timeouts as `timeout` instead of `other`. Failed eval items preserve finite numeric timeout evidence, and Studio keeps that evidence under redaction. Because the exported `TimeoutBreakdown` shape now has a required field, callers constructing it must update; ship this source-breaking type change in the next 0.x minor release.
