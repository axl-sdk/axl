...
| `adaptive` | `boolean` | Default `true`, and effective only on first-party OpenAI (`openai`, `openai-responses`) and Anthropic, meaning the vendor's default endpoint origin (`https://api.openai.com`, `https://api.anthropic.com`); a proxy or gateway `baseUrl` is unaffected. A rate-limit 429 pauses every call on the scope for its `Retry-After` (else the exponential backoff), clamped at 60 s, then retries on `maxRateLimitRetries`; the retrier re-acquires ahead of first-time callers. The same 429 also starts adaptive pacing: grants on the scope are spaced to a `rate` seeded at half the recent demand, cut once per wave, recovered linearly on success (held while quota headers are low), and dropped once traffic stays well below it ([providers.md → Adaptive pacing](providers.md#adaptive-pacing-after-a-rate-limit-429)). Its tuning is internal, never config, and spacing never goes below `minIntervalMs`. A spend-cap 429 is returned at once (no retry, no pause). `false` restores the plain behavior: a 429 shares the transient budget, holds up no other call, and nothing is paced. Two blocks on one scope: `true` wins. Ignored by a directly constructed `RateLimiter` |
