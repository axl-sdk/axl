...
- **Streaming run distinguishes user-cancellation from provider failure.** The terminal `done` event on a streaming run now carries `cancelled: true` when the user pressed Cancel and `batchFailure: <message>` when a real provider error stopped the batch. Pre-fix, both produced the same shape, so a cancelled partial batch landed in History looking like a provider hiccup. The route's catch block also detects `AbortError` from a future `runEval` that throws on signal abort (today's `runEval` returns cancelled-item results without throwing — both paths now produce the same wire signal). New `run_cancelled` event type is emitted from the catch path; `run_failed` is reserved for genuine errors.
- **`POST /api/evals/import` accepts arrays of EvalResults.** The CLI's `--output` writes a JSON array when `--runs N > 1`, including for partial batches (e.g. 2-of-5). Pre-fix the import endpoint required a single object, so a partial multi-run artifact couldn't be imported in one request — undermining the partial-batch story we worked to preserve. Now: array form imports each entry as its own history entry; per-run `runGroupId` flows through unchanged so the runs render as a coherent group in the History tab. Single-object form is still accepted (response shape unchanged for back-compat); array form returns `{ imported: [...] }`. Per-entry validation pinpoints the offending index when one element of a multi-run array is malformed. Studio's "Import result..." button handles both response shapes; status text shows the per-file run count when > 1. Blocked in `readOnly` mode.
- **Compare with mismatched run counts: stats consistency.** Pre-fix, `evalCompare` computed the per-scorer means over the full pool on each side (`baseline.length` and `candidate.length`) but the paired bootstrap CI was restricted to `min(baseline, candidate)` runs — so a 5-vs-2 pool showed a `delta` between means computed over different N next to a CI computed over only 2 paired diffs. The displayed numbers couldn't be cross-checked. Both sides now truncate to `min(baseline.length, candidate.length)` BEFORE means / regressions / timing / cost are computed, and `EvalComparison.baseline.runCount` / `.candidate.runCount` expose the truncation explicitly. Studio's compare view renders a blue banner when the truncation kicks in: "Comparing X baseline vs Y candidate runs. You pooled M and N; the comparison uses the first min(M, N) so the displayed means and paired bootstrap CI are computed over the same sample."
- **Renamed per-run `metadata.partialBatch` → `metadata.fromPartialBatch`.** The original name read as "this run is partial" and tempted defensive consumers to write `if (run.metadata.partialBatch) skipThisRun()` — discarding successful work that cost real money. The flag describes the BATCH the run came from, not the run itself; the new name makes that unambiguous. `batchCompleted` / `batchAttempted` / `batchFailure` are unchanged.
- **End-to-end test for the silent-passthrough removal.** Earlier coverage only verified the chain walk via synthesized module shapes (modern Node promotes named exports of CJS modules, so the original bug is hard to reproduce deterministically through real interop). The new test at `tests/e2e/scenarios/eval.test.ts` invokes the published CLI on a fixture with no `executeWorkflow` and an unregistered workflow, asserts non-zero exit, and pins three message guarantees: `does not export an executeWorkflow function`, mention of `"type": "module"`, and `Found exports`. A future regression that softens any of these silently would be caught.
- **Lifted broader CLI utilities to `@axlsdk/axl`.** `findConfig` / `CONFIG_CANDIDATES`, `needsTsxLoader`, `importModule`, `expandGlob`, and `registerConditions` were duplicated three ways (`packages/axl-eval/src/cli-utils.ts`, `packages/axl-studio/src/cli-utils.ts`, and a third private copy of glob expansion in `packages/axl-studio/src/eval-loader.ts`). Now share one implementation in `packages/axl/src/cli-internals.ts`, exported as `@internal` from `@axlsdk/axl`. The package-local `cli-utils.ts` modules are thin re-exports plus the package-specific arg parsers. Same anti-drift reasoning as the earlier `pickDefault`/`pickExport`/`resolveRuntime` lift — now applied to every loader-level utility.
- **Review-driven hardening of the partial-batch story.** Multi-perspective post-merge review surfaced two correctness gaps and several UX gaps in the partial-batch fix:
  - `detectPartial` (in `evalCompare`) and `buildMultiRunResult` (Studio client) read `runs[0].metadata.batchAttempted` only. If a user cherry-picks `[legacy_single_run, partial_batch_run]` from the comparison picker, run 0 has no batch metadata while run 1 does — the pre-fix behavior silently produced an "apples-to-apples" comparison. Both helpers now walk every run for the first finite `batchAttempted`, mirroring the existing `batchFailure` walk. `detectPartial` also returns `undefined` when `runs.length > batchAttempted` (data corruption signal) instead of silently treating it as complete.
  - `validateEvalConfig` (`axl-eval` CLI) used to accept any truthy values for `workflow`/`dataset`/`scorers`. Now rejects non-string `workflow`, non-object or array `dataset`, datasets without a `getItems()` method, and scorers arrays containing non-callable entries. The "executeWorkflow not found" error now lists the actual top-level + `.default` exports observed (`Found exports: [...]`) and explicitly mentions `"type": "module"` as the most likely root cause for users hitting this after the upgrade.
  - Empty `runFailure?.message` from `new Error('')` could produce a blank "Stopped after:" banner. CLI and Studio both coalesce to `String(runFailure)` now, then omit the field entirely if both are empty.
  - Trends chart's hollow-ring markers had no on-screen explanation. Added a "partial" legend chip alongside the existing color legend (rendered only when partial points are present in the chart) plus an SVG `<title>` on each partial circle so hover surfaces "Partial batch — &lt;label&gt; (fewer runs completed than planned)" without a tooltip framework.
  - Tests added: `validateEvalConfig` unit tests pinning every diagnostic string, edge-case `buildMultiRunResult` coverage (later-run batchAttempted, `batchAttempted: 0`, NaN), `detectPartial` walk-all-runs and corruption-signal coverage, RTL tests for the History `X/N runs PARTIAL` badge and the LineChart hollow-marker rendering, and a WS subscriber that pins the `run_done` → `run_failed` → terminal `done` event sequence on the streaming run endpoint.
- **DRY: lifted `pickDefault` / `pickExport` / `resolveRuntime` to `@axlsdk/axl`.** The original commit fixed an asymmetric ESM/CJS chain walk by introducing these helpers, then duplicated them across `packages/axl-eval/src/cli-utils.ts` and `packages/axl-studio/src/resolve-runtime.ts` (the two copies had already drifted in `any` vs `unknown` typing and inline vs extracted style). Both are now thin re-exports of `module-resolve.ts` in core, marked `@internal` in JSDoc since they're not part of the stable public surface. Future fixes to the interop walk apply to all consumers atomically — exactly the failure mode the original bug exposed.
- **`axl-eval` CLI / Studio eval-loader: symmetric ESM/CJS interop for named exports.** When `tsx` loads a `.ts` eval module from a package without `"type": "module"`, named exports (e.g. `executeWorkflow`) ended up under `mod.default.executeWorkflow` rather than `mod.executeWorkflow`. The default-export resolution already walked the interop chain (`mod.default?.default ?? mod.default ?? mod.config ?? mod`); the named-export check did not — so `evalConfig` resolved correctly while `executeWorkflow` did not, leading to silent fallback. Both call sites (`packages/axl-eval/src/cli.ts`, `packages/axl-studio/src/eval-loader.ts`) now go through a shared `pickExport(mod, key)` helper that walks the same chain, plus a paired `pickDefault(mod)` helper so the *default*-export chain is also extracted instead of inlined at three call sites. Future named exports inherit symmetric resolution by construction.
- **`axl-eval` / Studio eval-loader: validate `executeWorkflow` is a function.** A non-function `executeWorkflow` export (e.g. `export const executeWorkflow = 'foo'`) used to slip past the truthiness check and crash deep inside `runtime.trackExecution()`. The CLI now exits non-zero with a clear "expected function, got string" error; the studio loader logs a warning and falls back to `runtime.execute()`.
- **`axl-eval`: clearer error when the eval module's default export is not an object.** `export default 42` used to surface as `Cannot read properties of number (reading 'workflow')`. Now produces the standard "missing workflow, dataset, or scorers" error.
- **Partial-batch awareness in Compare and Trends.** The compare view and trends chart used to render partial-batch entries identically to complete ones — a 2-of-5 partial against a complete 5-run baseline produced a delta the user couldn't tell was computed against a smaller-N pool. `evalCompare` now returns a `partial?: { completed, attempted }` field on each side of the comparison (derived from `runs.length < runs[0].metadata.batchAttempted`); `EvalComparisonPartial` is exported from `@axlsdk/eval`. Studio's compare view renders an amber banner above the verdict ("Partial batch in this comparison. Baseline pooled 2 of an attempted 5 runs. Candidate pooled 3 of an attempted 5 runs. Statistics below are computed honestly for the included runs, but cross-side deltas reflect different sample sizes.") plus per-side `X/N` chips in the column headers. The trends chart renders partial points as hollow rings (background-fill, color-stroke) instead of solid dots; the line still passes through them so the trend is preserved. `EvalTrendRun` gains optional `runGroupId` / `batchAttempted` fields so the client can derive partial points by counting siblings sharing a group id and comparing against the planned count. `aggregateRuns` is unchanged — its `runCount` was already honest about sample size.
- **Studio: `POST /api/evals/:name/run` preserves partial multi-run batches.** Same bug class and fix as the CLI (see below). Both the streaming and synchronous paths used to throw on the first per-run failure, discarding completed runs the user already paid for. Each run is now stamped with `metadata.batchAttempted: N` up front (so persisted history records carry the planned count). On per-run failure: stop attempting, broadcast a `run_failed` event (streaming) or include `_multiRun.partial` markers in the response (sync), and exit cleanly. The Eval Runner UI surfaces partial batches at every level so they can't be mistaken for complete batches: an amber `X/N runs PARTIAL` badge in the History table row, and a partial-batch banner above the run-detail view (`Partial batch — X of N runs completed.` / `Stopped after: <reason>` / `Aggregate stats below are computed over the X completed runs.`). `buildMultiRunResult` propagates `batchFailure` from per-run metadata onto `_multiRun.batchFailure` so groups loaded from history show the failure cause, not just the count. `redactEvalResult` now scrubs `metadata.batchFailure` when redact mode is on, so CLI artifacts imported into a redact-mode Studio don't leak error strings.
- **`axl-eval`: multi-run mode (`--runs N`) preserves partial batches with explicit metadata.** When a run fails mid-batch, the original bug was that runs 0..N-1 stayed in the in-memory results with a `runGroupId` whose aggregate never ran, and `--output` would write a partial file *as if it were a complete batch* (no marker, sample size implied to be N). The fix: completed runs are still preserved (they cost real money and contain statistical signal), but each is tagged with `metadata.partialBatch: true`, `batchCompleted: <X>`, `batchAttempted: <N>`, and `batchFailure: <message>`. The aggregate is computed over what completed; the CLI prints a `PARTIAL: X of N runs completed` line and exits non-zero so CI knows the batch wasn't clean. We stop attempting further runs after the first failure (a transient rate limit or auth error speculatively retrying would be hostile). When *every* run fails (`runResults.length === 0`), no artifact is written and the file is reported as a per-file failure.
- **`@axlsdk/axl`: lifted the `EvalExecuteWorkflow` type to a single source of truth.** The function shape used by `registerEval`, `getRegisteredEval`, the `axl-eval` CLI, and the Studio eval-loader was inlined three times in `runtime.ts` and had already drifted (the parameter type omitted `metadata?` while the storage and getter included it). Now exported as `EvalExecuteWorkflow` from `@axlsdk/axl` and consumed by all four sites.
- **`axl-eval` / Studio: clean error when `tsx` is missing.** Previously: a `console.warn` followed by Node's cryptic `Unknown file extension ".ts"` error. Now `importModule` throws a single actionable message (`Cannot load TypeScript file <path>: tsx is not installed. Install it as a dev dependency: ...`) before Node has a chance to surface its own.
- **`axl-eval` / Studio: empty `scorers: []` array is now a load-time error.** The check used to be `!evalConfig.scorers`, which only catches `undefined`/`null` — an empty array passed and produced an eval with no scores at all.
- **`axl-eval` / Studio: invalid-config errors include diagnostic context.** Errors now list the missing fields specifically (e.g. `missing dataset, scorers`) and a `Got: { keys: ... }` trailer so users can spot a typo (`scorerS` vs `scorers`) immediately.
- **`axl-eval` CLI: glob patterns now expanded by the CLI.** `axl-eval 'evals/**/*.eval.ts'` (quoted to defeat shell expansion) was previously a "file not found" error. Single-directory (`dir/*.eval.ts`) and recursive (`dir/**/*.eval.ts`, `**/*.eval.ts`) globs now expand to matched files. Mirrors the studio eval-loader's semantics so users get the same behavior in both places — important on Windows / non-expanding shells.
- **`axl-eval` / Studio CLI: `--conditions development` now transforms chained `.ts` imports.** The previous `tsx/esm/api` `tsImport(specifier, parent)` approach scopes the loader hook to a single import call via a unique namespace — only the entry file gets transformed. Chained imports made by the entry (e.g. a workspace package whose `development` condition exports `./src/index.ts`) fall back to native Node ESM resolution, which throws `Unknown file extension ".ts"`. Switched to a one-time `register()` call (no namespace) so tsx's ESM hooks apply to the entire module graph for the process lifetime — matching what `tsx`'s own CLI does. Idempotent: registered on first TS file, reused for the rest of the process. Caveat: `--conditions` only fires on Node's ESM resolution pipeline (`module.register()` doesn't install CJS hooks). Files tsx compiles to CJS (`.cts`, or `.ts` in a package without `"type": "module"`) issue `require()` calls that bypass the hook — `import` syntax in the source is not sufficient. Workaround: add `"type": "module"` or rename the entry to `.mts`. The subprocess approach (`node --import tsx --conditions development`) would lift this limitation and remains future work.

### Breaking

- **`axl-eval` CLI: removed silent identity-passthrough fallback.** When neither an `executeWorkflow` export nor a registered workflow matched `evalConfig.workflow`, the CLI previously printed a `console.warn` and used identity passthrough — which silently produced all-zero scores in CI. The CLI now exits non-zero with a clear error explaining how to opt in (export `executeWorkflow` explicitly, or register the workflow with `runtime.registerWorkflow(...)`). If you genuinely want identity behavior, export it explicitly: `export const executeWorkflow = async (input) => ({ output: input });`.
- **`axl-eval` CLI: exits non-zero when any eval file fails to load or run.** Previously a mixed-success batch (e.g. `axl-eval evals/*.ts` with one broken file and three good ones) reported overall success as long as at least one eval finished. The CLI now exits 1 on any per-file failure, masking nothing in CI. Migration: if your CI tolerates per-file failures, split your invocation per-file or wrap the CLI to inspect output.
