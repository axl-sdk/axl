...
- Ask-level context summaries no longer overwrite durable session summaries or skip messages between a cached summary and a moving tail. Known client-side trims and summaries remove stale Anthropic thinking from retained requests while preserving other provider metadata.
