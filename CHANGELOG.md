...
- **`session.fork(newId)` no longer races a concurrent send on the target id.** Pre-fix, fork acquired only the source's lock and then wrote to `newId` — so a concurrent `runtime.session(newId).send(...)` could clobber or be clobbered by the fork. Fork now acquires both source and target locks (in lexicographic order, deadlock-free under crossed forks). Fork also now throws when `newId === source id` instead of silently doing the wrong thing.
