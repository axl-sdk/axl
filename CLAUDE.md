...
- `dataset()` extra-annotation-key detection: Zod object schemas strip undeclared keys by default, so an annotation field absent from the `annotations` schema is silently dropped before reaching scorers (the classic silent-no-op-scorer bug). `getItems()` diffs each item's raw annotations vs. the parsed result recursively (`collectDroppedKeys`, dot/bracket paths like `persona.role`) and reports per `onExtraAnnotationKeys?: 'warn' | 'error' | 'ignore'` (default `'warn'` — one consolidated `console.warn` per dataset; `'error'` throws; `'ignore'` skips detection). Mode-independent (strip/`.strict()`/`.loose()`) and source-independent (inline + `file:` JSON — the latter has no compile-time check at all). Scoped to `annotations` only, NOT `input` (input strips flow to the workflow where they surface). `ExtraKeyPolicy` exported from `@axlsdk/eval`. The type-system half (a `z.ZodType<T>` schema structurally narrower than `T` still satisfies the cast) is not robustly fixable in TS; this runtime warning is the backstop. Surfacing: the dataset captures the last `getItems()` drop set on `Dataset.droppedAnnotationKeys` (readonly getter, optional on the type so hand-rolled `{name, getItems}` test datasets stay valid); `runEval()` snapshots it after `getItems()` and folds it into `EvalResult.metadata.droppedAnnotationKeys` (only when non-empty). It rides through Studio's `redactEvalResult` (metadata passes untouched) and the REST/WS layers, and the Eval Runner panel renders an amber banner above the run stat cards via the `getResultDroppedAnnotationKeys()` client helper
