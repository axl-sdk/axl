...
| `adaptive` | `boolean` | Default `true` on every built-in chat provider (any `baseUrl`). A rate-limit 429 pauses every call on the scope for the exponential backoff (1 s, 2 s, 4 s, …), lengthened, never shortened, by a longer `Retry-After`/`retry-after-ms` (none observed from Gemini so far), clamped at 60 s, then retries on `maxRateLimitRetries`; the retrier re-acquires ahead of first-time callers. The same 429 also starts adaptive pacing: grants on the scope are spaced to a `rate` seeded at half the recent demand, cut once per wave, recovered linearly on success (held while quota headers are low, first-party OpenAI/Anthropic only), and dropped once traffic stays well below it ([providers.md → Adaptive pacing](providers.md#adaptive-pacing-after-a-rate-limit-429)). Its tuning is internal, never config, and spacing never goes below `minIntervalMs`. Spend caps are told apart only on first-party OpenAI (`openai`, `openai-responses`) and Anthropic at the vendor's default endpoint origin (`https://api.openai.com`, `https://api.anthropic.com`): there a spend-cap 429 is returned at once (no retry, no pause). Everywhere else (Gemini, OpenAI-compatible presets, a proxy or gateway `baseUrl`) the body is never read and every 429 is a rate limit, so a spend-cap or daily-quota 429 fails only once `maxRateLimitRetries` is spent. `false` restores the plain behavior: a 429 shares the transient budget, holds up no other call, and nothing is paced. Two blocks on one scope: `true` wins. Ignored by a directly constructed `RateLimiter` |
