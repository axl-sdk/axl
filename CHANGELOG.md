...
### Added
- **Providers: opt-in client-side rate governor (`RateLimitConfig`).** Provider pacing was purely *reactive* — the automatic 429/503/529 backoff only engaged after a rejection, so a large fan-out on a shared key (e.g. an eval's `concurrency × scorerConcurrency`, up to 25 concurrent judge calls) could storm a provider first. Set `rateLimit: { maxConcurrent?, minIntervalMs?, acquireTimeoutMs? }` on a provider config to install a dependency-free counting-semaphore + FIFO governor (`RateLimiter`, exported from `@axlsdk/axl`) at the single `fetchWithRetry` chokepoint, covering all four built-in adapters uniformly.
  - **Zero-regression:** with no `rateLimit`, `fetchWithRetry` is byte-identical to before. A permit is acquired once before the retry loop and released in `finally` (gated on an `acquired` flag), so the whole loop incl. backoff holds one permit and applies backpressure; a rejected/timed-out acquire never over-releases.
  - **Caps request concurrency, not token throughput** — a permit releases at response *headers*, so a streaming response doesn't hold it for the stream's lifetime.
  - **Chat calls only:** memory-embedder calls (constructed outside the registry) are out of scope in v1, and custom `registerInstance` providers aren't governed.
  - **Per provider instance / process:** `openai-responses` inherits the `openai` block's `rateLimit` but as a *separate* governor instance — using both adapters means the effective concurrency is the **sum**, not a shared counter; nothing is shared cross-process on one key.
  - `maxConcurrent: 1` serializes but does **not** deadlock a nested same-provider `ctx.ask()` (permits aren't held across a nested call — verified by test). `acquireTimeoutMs` fails loud instead of hanging on a misconfigured cap. `RateLimiter.observe()` is a no-op seam for a future adaptive (header-driven) follow-up.
  - **Minor API change:** `fetchWithRetry`'s 3rd argument is now an options object `{ maxRetries?, governor? }` (was a positional `maxRetries`; only internal callers + one test used it).
...
- **Eval: scorer failure-rate trust signal (catches the silent-thinned-sample trap).** When a judge call exhausts the provider's retry backoff it throws, the item's score becomes `null`, and the mean is computed over the **survivors** — so a `--fail-on-regression` gate can pass on a number drawn from half the dataset.
  - **Always-on signal:** the runner computes per-scorer `scored` / `failed` counts (a scorer skipped by cancellation is in neither bucket, so `scored + failed` is the honest "attempted" denominator) and surfaces them everywhere — the CLI table annotates `(18/20 scored, 2 failed)`, `axl-eval compare` always **warns** when a side rests on a thinned sample, and Studio shows an amber per-scorer badge in the summary table + score distribution.
  - **Two opt-in, type-aware gates** (deterministic scorers tolerate zero failures — a deterministic throw is a bug; LLM judges use the configured rate): a **source-side** `EvalConfig.failOnScorerErrorRate` (0–1) flags `summary.degraded` at run time — `runEval` never throws (it returns the still-useful result, so multi-run partials / auto-save / Studio refetch survive; the CLI maps a non-empty `degraded` to a non-zero exit) — and a **gate-side** `axl-eval compare --max-scorer-error-rate <0..1>` that refuses (exit 1) to certify an already-thinned or zero-sample baseline/candidate. An unknown/missing scorer type (old artifacts) is treated as LLM-permissive, not deterministic-zero, so an old baseline isn't hard-failed on its first flake. Invalid `failOnScorerErrorRate` is rejected at config load (symmetric with the flag). Opt-in, so existing `--fail-on-regression` pipelines are unaffected.
  - **New API:** exported `DegradedScorer` type and the pure helpers `evaluateScorerTolerance` / `evaluateScorerErrorRateGate` (build your own gate on `evalCompare`); new optional `EvalSummary.scorers[].{scored,failed}`, `EvalSummary.degraded`, and `EvalComparison.scorers[].{baseline,candidate}{Scored,Failed}` (old artifacts recompute from `items` via the shared `scoreDetails.duration` discriminator). `aggregateRuns` sums the counts across a multi-run group and excludes empty-sample runs from the aggregate mean (a 100%-failed run no longer drags the mean-of-means toward 0). Studio renders a `DegradedScorersBanner` ("(in N runs)" for multi-run groups).
