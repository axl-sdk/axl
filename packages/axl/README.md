...
On first-party OpenAI and Anthropic (their own endpoints, not a proxy `baseUrl`), a rate-limit 429 pauses every call on that account and model until `Retry-After` and retries on its own budget, while a spend-cap 429 fails fast; `rateLimit: { adaptive: false }` turns this off. Each provider also accepts an opt-in `rateLimit` (`{ maxConcurrent?, minIntervalMs?, acquireTimeoutMs?, adaptive?, maxRateLimitRetries? }`) for proactive client-side pacing on top of the automatic 429/503/529 handling — useful when a large fan-out (e.g. an eval) shares one API key. It caps in-flight request concurrency (not token throughput) for that provider's chat calls. See [Providers → Rate limiting](../../docs/providers.md#rate-limiting).
