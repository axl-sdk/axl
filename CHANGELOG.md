...
- **Cross-process session race surfaced in three places.** The "in-process locking only" caveat now appears on the `Session` class JSDoc, in `docs/architecture.md` directly under the RedisStore example (where users copy-paste the multi-worker setup), and in `docs/api-reference.md` Sessions → Concurrency. Previously buried in a single api-reference paragraph; users following the architecture guide to wire up Redis would not have seen the warning.
