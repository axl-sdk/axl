...
| `effort` | `Effort` | — | Unified effort level: `'none'` \| `'low'` \| `'medium'` \| `'high'` \| `'xhigh'` \| `'max'`. Exact model and endpoint capabilities decide whether a tier is sent, clamped, or omitted. GPT-5.6 supports native `'max'` on Responses while Chat Completions warns once per exact model and caps it to `'xhigh'`; Claude 5 supports native `'max'`; earlier families retain their documented caps |
