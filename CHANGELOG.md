...
- **`ctx.events.partialObjects` is schema-retry-aware.** Each yielded value now carries `attempt: number` so UIs can flag a regenerating draft. When a schema check fails and the agent retries, the runtime emits `pipeline(status: 'failed')` and the view drops any undrained pending partial for that ask — so the consumer never sees stale attempt-N snapshots after attempt-N+1 has begun. Mirrors the per-attempt buffer reset that `AxlStream.fullText` applies to tokens. Validate-stage and guardrail retries trigger the same reset. Same shape change applies to `AxlStream.partialObjects`.
- **`Session.send` / `Session.stream` accept `{ events: EventStreamOptions }`.** Configures the iterator-queue cap and overflow policy on the underlying workflow's `ctx.events` bus (and on the `AxlStream` returned by `session.stream`). Closes a consistency gap with `runtime.execute` / `runtime.stream` / `runtime.createContext`. Also fixed: `session.stream` now correctly forwards `signal` to `runtime.stream` (was previously dropped silently — pre-existing bug exposed during this work).
- **`AxlTestRuntime.execute` accepts `{ events: EventStreamOptions }`.** Tests can now exercise the queue cap and overflow policy on `ctx.events` end-to-end without dropping to direct bus construction. Mirrors the production runtime API.
- **`AxlEventBus` validates `maxQueued`.** Constructor throws `Error` on `0`, negative numbers, and `NaN` — values that previously silently disabled the cap. Pass `Infinity` to disable explicitly.
- **`onOverflow: 'throw'` on `AxlStream` now correctly propagates as a typed `EventStreamOverflowError`.** Pre-existing bug exposed during the gap-fix pass: the runtime's internal `onTrace` handler wraps `axlStream._push` in a try/catch (to isolate buggy user trace listeners), which was also swallowing the strict-mode overflow throw. The bus now throws a dedicated `EventStreamOverflowError` (exported from `@axlsdk/axl`) and `WorkflowContext.emitEvent` re-throws this specific class through the trace-listener guard so it reaches the workflow's promise rejection. Strict-mode users who set `onOverflow: 'throw'` on `runtime.stream(...)` finally see the workflow fail loudly on saturation as documented. Catch with `instanceof EventStreamOverflowError`.
- **`ctx.events` auto-disposes on signal abort.** When `runtime.createContext({ signal })` is constructed with an `AbortSignal` and the signal fires, the bus is auto-finished — iterators terminate with `done: true`. Pre-aborted signals trigger disposal on first `ctx.events` access. Closes the long-lived ad-hoc context leak path: a consumer that iterates `ctx.events` and never sees a workflow terminal would otherwise hang forever.
- **`ctx.events` — stream-first observation surface on every `WorkflowContext`.** Iterate events between `ctx.ask()` calls inside a workflow handler — the customer use case driving the design. Same `AxlEvent` union and curated views as `AxlStream` (`.text`, `.lifecycle`, `.textByAsk`, plus the new `.partialObjects` coalescing view), scoped to the current context. Lazy: zero overhead unless a consumer subscribes. Auto-terminates on `workflow_end` / `error`. Child contexts (agent-as-tool nested asks) share the parent's bus through a mutable slot, so a late subscription on the parent still receives events from a child that already exists. The streaming code path inside `ctx.ask()` now activates whenever an observer is present (legacy `onToken` OR `ctx.events` allocated), so `runtime.execute()` flows that subscribe to `ctx.events.partialObjects` actually receive `token` and `partial_object` events. The `onToken` / `onToolCall` / `onAgentStart` callbacks on `runtime.createContext()` remain for back-compat but are superseded by the iterable. New on `runtime.execute()` / `runtime.stream()` / `runtime.createContext()`: `events?: EventStreamOptions` to configure the iterator-queue cap and overflow policy. New `AxlEventBus` and `EventStreamOptions` exports from `@axlsdk/axl`. Tracked in `ROADMAP.md` as "Stream-First Observation API" — moved from Planned to Complete.
- **Coalescing `partialObjects` view on `AxlStream` and `ctx.events`.** Yields the latest `partial_object` payload per `askId` — when the producer outpaces the consumer, intermediate snapshots are silently superseded. Memory bound is `O(active asks)`, not `O(events emitted)`. Listener-based, so it does NOT race with the main async iterator: running `for await (const e of stream)` and `for await (const p of stream.partialObjects)` concurrently each receive every event. Designed for streaming-structured-output UIs that render the current state, not every intermediate snapshot.
- **Bounded queue + overflow policy on `AxlStream` and `ctx.events`.** Default cap of `10_000` events with `onOverflow: 'drop-oldest-non-terminal'`. Terminal events (`done`, `error`, `workflow_end`) always pass through. First overflow per bus emits a one-shot `console.warn`. Default-on safety net: replaces silent OOM under sustained slow-consumer pressure with graceful degradation + a visible signal. Existing `AxlStream` consumers gain the cap automatically; opt out with `events: { maxQueued: Infinity }`. Strict environments can pick `onOverflow: 'throw'` to fail the workflow on saturation rather than drop.
...
- **`Session.fork` copies session-scoped key-value memory.** Before this fix, forking a session preserved `history` + `summaryCache` + `handoffHistory` but silently dropped session-scoped memory entries — the forked conversation would "forget" things the source had remembered via `ctx.remember(..., { scope: 'session' })`. Vector embeddings are still not copied (re-embed on the fork if you need semantic recall there); KV entries now carry over for any StateStore implementing `getAllMemory`/`saveMemory` (MemoryStore, SQLiteStore).
- **Multi-agent sessions: assistant messages now carry the originating agent name.** When a workflow calls `ctx.ask(triageAgent, ...)` followed by `ctx.ask(billingAgent, ...)` in one session, you can now see which agent produced each reply. Every committed assistant message is stamped with `ChatMessage.agent: 'triageAgent'` (or whichever agent's `name`) in persisted history. The Studio Session Manager panel surfaces this as a clickable badge next to each assistant turn — clicking the badge highlights all messages from that agent so multi-agent conversations are easy to follow visually. The field is observability-only today; per-agent history filtering is planned for a future release. Backward compatible: existing serialized sessions without the field deserialize cleanly, and the field never appears on outbound provider HTTP payloads (verified via wire-payload tripwire tests against OpenAI / Anthropic / Gemini / OpenAI-Responses). Studio's `redactChatMessage` preserves the agent name even in redact mode (it's a non-PII config-time identifier, like a tool name).
- **Lock-contention observability for sessions.** Listen for `runtime.on('session_lock_contended', ({ sessionId }) => ...)` to detect when concurrent calls are queueing on the same session id. Useful for debugging "why is my session slow?" without guessing.

### Fixed

- **Concurrent `session.send()` calls no longer lose messages.** Before this fix, two parallel `runtime.session('x').send(...)` calls on the same session id could lose an entire user/assistant exchange — both would read the same history snapshot, append independently, and the later writer would clobber the earlier one. `Session.send`, `stream`, `end`, and `fork` now serialize per session id within the runtime; concurrent calls queue FIFO behind the in-flight task. `end()` no longer "resurrects" a session by racing the in-flight save, and `fork()` captures a committed snapshot rather than a torn pre-save state. `runtime.shutdown()` drains in-flight session work before closing the state store. Two `runtime.session('x')` handles share the same lock (it lives on the runtime, not the `Session` instance). **Cross-process locking is NOT provided** — horizontally-scaled deployments must pin sessions to workers (sticky routing on `sessionId`) or use distinct ids per request; see `docs/api-reference.md` Sessions → Concurrency for details.
- **`session.fork(newId)` no longer races a concurrent send on the target id, and no longer silently overwrites existing data.** Pre-fix, fork acquired only the source's lock and then wrote to `newId` — so a concurrent `runtime.session(newId).send(...)` could clobber or be clobbered by the fork. Fork now acquires both source and target locks (in lexicographic order, deadlock-free under crossed forks). Fork also throws when `newId === source id`, AND throws when `newId` already has history unless the caller passes `{ overwrite: true }` — preventing accidental data loss when forking into a session id that's already in use.
- **`pnpm dev:studio` works under pnpm's strict isolation.** When `tsx watch src/cli.ts` (axl-studio's dev mode) launched the CLI, `importModule` (the shared `.ts` config loader in `@axlsdk/axl`) was preemptively trying to register tsx hooks via `await import('tsx/esm/api')`. tsx isn't symlinked into axl's own `node_modules` under pnpm — only into the consumer (axl-studio / axl-eval) — so the registration failed and the CLI errored with "tsx is not installed" even though the parent process had tsx fully active. Fix: attempt the dynamic import first; only fall back to registration when Node surfaces a stable missing-loader signal (`ERR_UNKNOWN_FILE_EXTENSION` code, or the `Cannot use import statement outside a module` message for the CJS branch). The actionable "tsx is not installed" error is preserved for the genuine no-loader-anywhere case.

### Documentation

- **Stream-First Observation API documentation polish.** Added [migration guide](docs/migration/stream-first-observation.md) covering all behavior changes (default `AxlStream` queue cap, `runtime.execute` streaming activation when `ctx.events` is allocated, `Session.stream` signal forwarding fix, `EventStreamOverflowError` propagation). Root README now includes a `ctx.events` snippet under Streaming so the new feature is discoverable from page one. `partialObjects` shape across all docs updated to include the `attempt` field (was missing). Examples in README, `packages/axl/README.md`, `docs/observability.md`, `docs/api-reference.md` now attach `.catch` to the background-observer IIFE so consumer errors don't surface as unhandled rejections. Cross-references between `ctx.events` mentions and the canonical reference. `packages/axl-eval/README.md` documents per-item live observation via `ctx.events` inside an eval's `executeWorkflow` export. `packages/axl-testing/README.md` documents `events: EventStreamOptions` config on `AxlTestRuntime.execute`. `packages/axl-studio/README.md` notes the relationship between Studio's `runtime.on('trace')` consumer and `ctx.events`. `docs/testing.md` adds recipes for testing `partialObjects` coalescing, overflow policy, and schema-retry pollution. `CoalescedPartialObject` extracted as a shared type alias and exported from `@axlsdk/axl` so the bus and stream surfaces can never drift on the yielded shape.
- **Cross-process session race surfaced at every doc touchpoint a user could land on.** The "in-process locking only" caveat now appears on the `Session` class JSDoc, in `docs/architecture.md` directly under the RedisStore example (where users copy-paste the multi-worker setup), under the `session.send` Express handler example in `docs/integration.md` (the canonical webserver-embed pattern), and in `docs/api-reference.md` Sessions → Concurrency. Previously buried in a single api-reference paragraph; a user following the architecture guide to wire up Redis or the integration guide to embed in Express would not have seen the warning at the point of copy-paste.
- **Filter-by-agent example in `docs/use-cases.md`.** The whole point of `ChatMessage.agent` is consumer-side attribution, but no doc showed how to actually use it. Added a 12-line snippet under the Agent Triage section showing `history.filter((m) => m.agent === 'BillingBot')` and per-agent grouping for analytics.
- **Sessions: documented sharing semantics, the "turn" definition, lock contract, and the multi-process footgun.** The Sessions section in `docs/api-reference.md` previously listed only options and methods. Added subsections: "What's stored" (flat `ChatMessage[]` keyed by `sessionId`, the `Session` object holds no message cache), "Sharing semantics" (history is session-scoped, not agent- or workflow-scoped — but multi-agent assistant turns now carry `ChatMessage.agent`), "Turns vs. exchanges" (a "turn" in axl is one provider call inside `ctx.ask()`, not a user↔assistant round-trip), "Concurrency and races" (per-session FIFO lock contract, the `session_lock_contended` event, and a prominent **warning block** that cross-process locking is NOT provided — horizontally-scaled deployments must pin sessions to workers or use distinct ids), and "Summarization" (session-level vs ask-level paths are independent). Method table now flags which methods serialize and which read snapshots. `AgentConfig.maxTurns` annotated with a pointer to the Turns vs. Exchanges note. README files updated to mention `ChatMessage.agent` and the per-session lock contract. Reported by a user who couldn't tell from the docs whether sessions partitioned by workflow.
