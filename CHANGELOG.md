...
- **AWS Bedrock preset (`bedrock:`).** gpt-oss via Bedrock's OpenAI-compatible endpoint with bearer auth (`AWS_BEARER_TOKEN_BEDROCK`, no SigV4). Region-specific base URL (prefer the `bedrock-mantle` endpoint; `requireExplicitBaseUrl`), versioned model ids (`bedrock:openai.gpt-oss-120b-1:0`), `reasoning_effort` for gpt-oss, cost `unknown` (Bedrock returns no per-call cost). Claude-on-Bedrock is deferred to a later native-adapter mode.
