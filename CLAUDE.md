...
import { AxlEventBus, EventStreamOverflowError, stringStreamFromEvents } from '@axlsdk/axl';
import type {
...
packages/axl/src/
  ...
  stream.ts          — AxlStream (Readable + composes AxlEventBus for iterator/EventEmitter/views)
  event-stream.ts    — AxlEventBus (iterator + EventEmitter + curated views: .text, .lifecycle, .textByAsk, .partialObjects, .stringStream), EventStreamOptions (maxQueued + onOverflow). Shared between AxlStream and ctx.events
  streaming-walker.ts — Path-tracking JSON state machine driving partial_object + string_delta emissions during schema-mode `ctx.ask()` streaming
...
packages/axl-eval/src/
  ...
  dataset.ts         — dataset() factory with inline/file loading + extra-annotation-key detection
...
packages/axl-studio/src/
  ...
  client/
    ...
    panels/
      ...
      eval-runner/   — Eval execution + comparison (EvalSummaryTable, EvalItemList, EvalItemSidebar, EvalItemDetail, ScoreDistribution, EvalCompareView, EvalHistoryTable, EvalCompareItemTable, EvalCompareRunPicker, EvalMultiRunSwitcher, EvalTrendsView — By Scorer/By Model/Duration views with click-to-run-detail). Score colors: 3-tier system (>=0.8 green, >=0.5 amber, <0.5 red). Run detail shows an amber "annotation keys dropped by schema" banner above the stat cards when `metadata.droppedAnnotationKeys` is non-empty
```
...
- `effort` is the unified cross-provider param (`'none'|'low'|'medium'|'high'|'xhigh'|'max'`); `'none'` disables thinking/reasoning. `'xhigh'` is a first-class tier between `'high'` and `'max'`, supported natively on Anthropic Opus 4.7 and OpenAI gpt-5.2+; clamps to `'high'` elsewhere. Maps to: reasoning_effort (OpenAI o-series + GPT-5.x, `'max'`→`'xhigh'`), adaptive thinking + output_config.effort (Anthropic 4.7/4.6, including `'xhigh'` on 4.7 only), output_config.effort only (Anthropic 4.5), budget_tokens fallback (Anthropic older), thinkingLevel (Gemini 3.x — `'xhigh'`→`'high'`), thinkingBudget (Gemini 2.x — `'xhigh'`→16384). OpenAI effort values are clamped per model: `'none'`→`'minimal'` on pre-gpt-5.1 (which don't support `'none'`), `'xhigh'`→`'high'` on pre-gpt-5.2, and always `'high'` on gpt-5-pro
...
- Anthropic Opus 4.7 + 4.6 models (Opus 4.7, Opus 4.6, Sonnet 4.6) use adaptive thinking (`thinking: { type: "adaptive" }` + `output_config: { effort }`) when `effort` is set. Opus 4.7 additionally supports `effort: 'xhigh'` as a distinct tier; on Opus 4.6/Sonnet 4.6 it clamps to `'high'` via `clampAnthropicEffort()`. Opus 4.5 supports `output_config.effort` but not adaptive thinking. `thinkingBudget` falls back to manual mode (`thinking: { type: "enabled", budget_tokens }`) for precise control. `effort` + `thinkingBudget: 0` sends standalone `output_config.effort` without thinking block. Helpers: `supportsAdaptiveThinking` matches 4.7/4.6 prefixes; `supportsMaxEffort` matches Opus 4.7/4.6; `supportsXhigh` matches Opus 4.7 only; `supportsEffort` matches 4.7/4.6/4.5
...
- State: `StateConfig.store` accepts `'memory'` | `'sqlite'` | `StateStore` instance. `'redis'` is NOT a valid string — pass `await RedisStore.create(url)` as the instance. RedisStore requires the `redis` peer dep (node-redis v5, not ioredis). Private constructor enforces async factory usage. `StateConfig` also accepts `persist?: 'terminal' | 'streaming'` (default `'terminal'`), `streamingBatchSize?: number` (default 100), `streamingBatchInterval?: number` (default 1000ms), `maxEventsPerExecution?: number` (default 50k)
- StateStore optional methods: `saveExecution`/`getExecution`/`listExecutions`/`deleteExecution` for execution history persistence, `saveEvalResult`/`listEvalResults`/`deleteEvalResult` for eval history, `appendStreamingEvents`/`finalizeStreamingEvents`/`listStreamingExecutions`/`getStreamingEvents` for `state.persist: 'streaming'` durability, `saveMemory`/`getMemory`/`getAllMemory`/`deleteMemory` (RedisStore now implements these — was a sessionMeta fallback pre-0.17.7 that dropped the `metadata` option). All 3 built-in stores implement the history + delete methods. RedisStore + MemoryStore implement streaming methods; **SQLiteStore does NOT** implement streaming (one-shot warning at runtime startup; configure `persist: 'terminal'`). Completed/failed executions and eval results auto-persist; lazy-loaded on first access. With SQLite/Redis, history survives restarts
- **RedisStore production hardening (0.17.7):** `RedisStore.create(url | { url, keyPrefix?, defaultTtl?, ttls?, skipMigration? })`. Default `keyPrefix: 'axl:'` (back-compat); empty string rejected; glob metacharacters in prefix are accepted but break `redis-cli SCAN MATCH` patterns. `defaultTtl` (seconds) is the per-category default; `ttls.<category>` overrides per-category — categories: `session`, `sessionMeta`, `checkpoint`, `executionState`, `executionHistory`, `evalHistory`, `memory`, `streamingEvents`. **`streamingEvents` is opt-in only** (no `defaultTtl` fallback); must be longer than max process-restart-gap or recovery misses. **Window semantics:** sliding (`memory`/`session`/`sessionMeta` — every write refreshes, reads DO NOT), fixed-creation (`checkpoint`/`streamingEvents` — `EXPIRE NX` on first write), fixed-refresh (`executionState`/`executionHistory`/`evalHistory` — `SET ... EX` always refreshes). `pendingDecision` is intentionally NOT configurable (shared `axl:decisions` hash). Non-finite/non-positive TTL values map to no-TTL defensively. Every multi-key write wraps in `MULTI/EXEC` (atomicity — no half-committed state). `listExecutions`/`listEvalResults` use sorted-set fast path (reverse `ZRANGE ... BYSCORE` + `MGET` — node-redis v5 removed the dedicated `zRevRangeByScore` method); over-fetch by 2× (+5 floor) so TTL drift doesn't undercount. Lazy backfill on startup populates ZSET from legacy SET — idempotent, opt-out via `skipMigration: true` for six-figure deployments. `listPendingExecutions` self-prunes ids whose state blob TTL'd. Legacy memory entries migrate forward on first read (race-safe HSETNX) with the configured TTL applied
- **`state.persist: 'streaming'` durability:** Events are batched (`StreamingFlusher`) and flushed to `StateStore.appendStreamingEvents` throughout the run via two triggers: size (`streamingBatchSize`) and time (`streamingBatchInterval`). Finalized when canonical `executionHistory` blob lands at terminal exit; save-failure preserves buffer for next-process recovery. `STREAMING_EXCLUDED_TYPES = {token, partial_object, string_delta}` — high-volume stream-only events never flushed (reconstructable from `agent_call_end.data.response`). **Scope: `runtime.execute()` and `runtime.stream()` only** — `runtime.createContext()` flows do NOT register in `streamableExecutionIds` (no terminal finalize path; allowing them would leave phantom orphans for `recoverIncompleteStreams()` to mis-recover). `runtime.shutdown()` drains the flusher AND awaits all in-flight `persistExecution` chains (tracked via `persistInflight: Set<Promise>`) before closing the store. After a crash, `runtime.recoverIncompleteStreams(): Promise<ExecutionInfo[]>` reconstructs partial executions: synthesized `status: 'failed'`, `error: 'process terminated (recovered from streaming buffer)'`, `workflow: '__axl/recovered'` (the `RECOVERED_UNKNOWN_WORKFLOW` sentinel) when no `workflow_start` was captured. Safety: skips ids in `this.executions` / `streamableExecutionIds` (live-execution guard), bounds events to `maxEventsPerExecution` (via `boundRecoveredEvents` with the same truncation sentinel as `pushEventBounded`), preserves buffer on `saveExecution` failure
- **`runtime.deleteExecution(id): Promise<boolean>`:** GDPR right-to-be-forgotten. Removes from `this.executions`, `this.historicalExecutions`, AND every per-execution `StateStore` surface in one atomic call (canonical row + indexes + checkpoints + suspended state + pending-set membership + streaming buffer + streaming-ids set + pending decision hash field). In-flight handling: aborts the workflow via the registered controller AND adds the id to `pendingDeletedExecutions: Set<string>` so the workflow's eventual `workflow_end` short-circuits `persistExecution` (no resurrection). Eagerly clears `streamableExecutionIds` so wind-down events don't re-create the buffer between deleteExecution returning and persistExecution running. Clears `pendingDecisionResolvers.delete(id)` to prevent resolver-closure leak. Emits `execution_deleted` on the runtime EventEmitter with `{ executionId, workflow, wasActive, hadPendingDecision, removed }` (fires on every call, including unknown-id attempts with `removed: false`; `workflow` captured BEFORE delete via the lazy-load lookup, undefined for unknown ids). Symmetric: `runtime.deleteEvalResult(id)` emits `eval_deleted` with `{ id, eval, removed }`. Studio's three aggregators (`TraceAggregator`, `ExecutionAggregator`, `EvalAggregator`) all subscribe to the delete events and trigger an immediate rebuild — without this, deleted runs linger in Cost Dashboard / Workflow Stats / Trace Stats / Eval Trends snapshots for up to 5 minutes (the periodic rebuild interval). Studio's `DELETE /api/executions/:id` wraps it AND calls `ConnectionManager.clearChannelBuffer('execution:{id}')` to scrub the WS replay buffer (otherwise late subscribers replay deleted events for ~30s); blocked in `readOnly` mode (405 with `error.code: 'READ_ONLY'`). **Custom store implementors:** `StateStore.deleteExecution?` is the canonical total-sweep entry point — must drop checkpoints, execution_state, pending decisions, streaming buffer, AND the canonical row in one call (transaction / MULTI). The runtime no longer calls separate cleanup methods
- **`ExecutionInfo.metadata`:** Lifted from `ExecuteOptions.metadata` / `runtime.stream()` `options.metadata`. Internal control-plane keys (`INTERNAL_METADATA_KEYS = {sessionHistory, sessionId, resumeMode}`) are stripped by `liftPersistedMetadata` before lift so they don't bloat the persisted snapshot — they remain readable on `ctx.metadata` for dynamic selectors. The snapshot is `structuredClone`'d so caller mutations to `options.metadata` after execute() don't surface mid-run. Non-cloneable values (functions) fall back to a shallow copy at lift; `persistExecution` then has its own `structuredClone(execInfo)` wrapped in try/catch with a `sanitizeMetadataForPersist` fallback that drops uncloneable keys — workflow never crashes on exotic metadata. SQLite schema v3 adds an `execution_history.metadata` TEXT column (auto-migrated on first open). Studio's `redactExecutionInfo` scrubs metadata to `{ redacted: true }` when `config.trace.redact: true` (caller-supplied userId/tenantId are PII surfaces)
- **`ctx.awaitHuman()` wakes on signal abort (0.17.7 fix):** `_awaitHumanImpl` races the `pendingDecisions.set(id, resolve)` registration against `this.signal`. Fast-path check for already-aborted before register; `signal.addEventListener('abort', ...)` removes the registration (only if it's still ours — guards against `resolveDecision` having already cleared it) and rejects with `AbortError`. Fired by `runtime.abort(id)`, `runtime.deleteExecution(id)`, `options.signal`, or a budget hard-stop. Pre-0.17.7 a paused awaitHuman hung forever on abort
...
- `dataset()` extra-annotation-key detection: Zod object schemas strip undeclared keys by default, so an annotation field absent from the `annotations` schema is silently dropped before reaching scorers (the classic silent-no-op-scorer bug). `getItems()` diffs each item's raw annotations vs. the parsed result recursively (`collectDroppedKeys`, dot/bracket paths like `persona.role`/`tags[0].label`) and reports per `onExtraAnnotationKeys?: 'warn' | 'error' | 'ignore'` (default `'warn'` — one consolidated `console.warn` per dataset; `'error'` throws; `'ignore'` skips detection). **Detection only runs when the annotations schema is a `ZodObject`** (`isObjectSchema()` via the public `.shape` getter): strip/`.strict()`/`.loose()` are all handled, but a schema wrapping `.transform()`/`.preprocess()`/pipe (or a union/record) is skipped — diffing reshaped parse output would mis-report intentionally-removed keys as "dropped" (and hard-fail under `'error'`). Residual: a `.transform()` nested inside an object field can still false-positive that sub-path (rare; documented, not worth a full schema walk). Source-independent (inline + `file:` JSON — the latter has no compile-time check at all). Scoped to `annotations` only, NOT `input` (input strips flow to the workflow where they surface). The annotation parse gate is `!= null` (nullish), so falsy-but-defined annotations are now validated rather than passed through raw. `ExtraKeyPolicy` exported from `@axlsdk/eval`. Type-system note: on Zod v4 TS catches an inline-literal excess key AND a `z.ZodType<T>` schema narrower than `T` (a compile error) — only variable-sourced items, `file:` JSON, and `as unknown as` casts evade it, which is what this runtime backstop covers. Surfacing: the dataset captures the last `getItems()` drop set on `Dataset.droppedAnnotationKeys` (readonly getter returning a defensive copy, optional on the type so hand-rolled `{name, getItems}` test datasets stay valid); `runEval()` snapshots it after `getItems()` and folds it into `EvalResult.metadata.droppedAnnotationKeys` (only when non-empty). It rides through Studio's `redactEvalResult` (metadata passes untouched) and the REST/WS layers, and the Eval Runner renders `DroppedAnnotationKeysBanner` (amber, `role="status"`, chips capped + "+N more") above the stat cards in BOTH the single-run and multi-run-aggregate views via `getResultDroppedAnnotationKeys()`
...
- `LlmScorerConfig` exposes the full `ChatOptions` surface: `temperature` (default 0.2), `maxTokens`, `effort`, `thinkingBudget`, `includeThoughts`, `stop`, `providerOptions` — all forwarded verbatim to `provider.chat`. Use `effort: 'high'` for reasoning-capable judges (gpt-5.x, Opus 4.6+, Gemini 3.x) — materially improves calibration. `responseFormat: { type: 'json_object' }` stays hardcoded. `ScorerContext` is typed against the real `Provider` interface (single source of truth — any future `ChatOptions` field is auto-available to scorers) and includes `signal?: AbortSignal` propagated from `RunEvalOptions.signal` so judge calls abort mid-flight on cancellation
...
- `ctx.events` (every WorkflowContext): lazy `AxlEventBus` exposing the same `AxlEvent` iterable + curated views as `AxlStream` (`.text`, `.lifecycle`, `.textByAsk`, `.partialObjects`). Allocated on first access (zero overhead otherwise). Auto-terminates on `workflow_end` / `error`; ad-hoc `runtime.createContext()` flows that never emit a terminal can call `ctx.disposeEvents()`. Child contexts share the parent's bus via a mutable slot `_busRef: { current?: AxlEventBus }` — late allocation in either is visible to all (parent + nested-ask children all read/write the same slot). Streaming code path inside `ctx.ask()` activates whenever an observer is present (legacy `onToken` OR `_busRef.current` exists), so `runtime.execute()` flows that subscribe to `ctx.events.partialObjects` actually receive `token` / `partial_object` events. **Subscribe before the first `ctx.ask()`** — the streaming gate is re-checked per ask, so a late subscription means only the in-flight ask misses tokens/partials; subsequent asks stream normally. `AxlEventBus` and `EventStreamOptions` exported from `@axlsdk/axl`. Configure via `events?: EventStreamOptions` on `runtime.execute()` / `runtime.stream()` / `runtime.createContext()`
- `partialObjects` coalescing view (on both `AxlStream` and `ctx.events`): yields the latest `partial_object` payload per `askId`. Listener-based, so it does NOT race with the main async iterator (`for await (const e of bus)` and `for await (const p of bus.partialObjects)` each receive every event). Memory bound is `O(active asks)`, not `O(events emitted)`. Snapshots existing queued partial_objects at subscribe time so consumers iterating after a burst still see the latest. Filters malformed events (those missing `data.object`)
- **`stringStreamFromEvents(source, opts?)`** is the browser-safe wire-side reconstructor for the `stringStream` view, exported from `@axlsdk/axl`. Pure ECMAScript, takes an `AsyncIterable<AxlEvent>` (from a WS/SSE adapter on the client), yields `AsyncIterable<StringStreamEvent>` with the same `{ path?, askId? }` filter shape. Maintains the same per-(askId, path) accumulator client-side and clears on `pipeline(failed)`/`ask_end` like the bus view. **No late-subscriber seeding** (no shared accumulator on the wire) and **no pending-buffer drop on retry** (events yield as they arrive — but the retry-clear of the accumulator means the next yielded event's `accumulated === delta`, which naturally overwrites stale text when the consumer re-renders). Recipe in `docs/observability.md`.
- **`string_delta` AxlEvent variant + `stringStream` view (spec/17, 0.17.5).** Per-chunk character-level deltas inside string VALUES of progressive structured output, keyed by RFC 6901 JSON Pointer (`/summary`, `/sources/0/title`). Same gating as `partial_object` (schema set, no tools, root is `ZodObject`). Stream-only, not persisted to `ExecutionInfo.events`. The unified `StreamingWalker` (`packages/axl/src/streaming-walker.ts`) is a path-tracking JSON state machine that replaces the inline `partial_object` walker and drives BOTH emissions from one parser. Walker also fixes a latent prefix-prose bug in the old walker (a `"` in pre-JSON prose flipped the in-string flag). Walker is reset per agent-call (per turn) — fresh state on schema retries. Escape sequences (`\\n`, `\\"`, `\\\\`, `\\uXXXX`, surrogate pairs) translated before emission; escape splits across chunks supported via persistent `unicodePending`/`escaped` state. `MAX_DEPTH = 256` adversarial-nesting guard (mirrors `parsePartialJson`). `StringStreamData` carries `{ path, delta }`; redaction scrubs `delta` only (path is structural, never PII). `bus.stringStream({path?, askId?})` returns `AsyncIterable<StringStreamEvent>` (`{ askId, agent?, path, delta, accumulated, attempt }`). `delta` is new chars; `accumulated` is running text-so-far for that ask + path. Late subscribers seeded with one synthetic event carrying `delta === accumulated === <full text-so-far>`. Listener-based: does NOT race the main iterator. Per-ask accumulator at the bus level: `Map<askId, Map<path, {agent?, text, attempt}>>` — cleared on `pipeline(failed)` (discarded attempt) AND `ask_end` (memory hygiene; string buffers can be KB+, asymmetric to `latestPartialByAsk` which doesn't clear on ask_end because snapshots are tiny). `_push` updates accumulators BEFORE `bus.emit(...)` so live listeners reading `accumulated` see post-this-event state — also exception-safe (accumulator update survives listener throws). `string_delta` is excluded from Studio's WS replay buffer (`UNBUFFERED_EVENT_TYPES` in connection-manager.ts) — late subscribers reconstruct field state via the bus accumulator, not via replay. Studio Playground filters `string_delta` from the activity feed alongside `token` and `partial_object`. Designed for chat-style typewriter rendering of long string fields. Recipe in `docs/observability.md` (`StreamingField` React component pattern).
- Bounded queue + overflow policy on `AxlEventBus` (and therefore on every `AxlStream` and `ctx.events`): `EventStreamOptions = { maxQueued?: 10_000, onOverflow?: 'drop-oldest-non-terminal' (default) | 'throw' }`. Terminal events (`done`, `error`, `workflow_end`) bypass the cap. First overflow per bus emits a one-shot `console.warn`. `'throw'` policy throws inside `_push` → `emitEvent` → unwinds active ctx.* primitive → fails the workflow (documented). Default-on safety net; opt out with `maxQueued: Infinity`
...
- Studio: `POST /api/evals/import` accepts `{ result: EvalResult | EvalResult[], eval? }` to ingest a CLI artifact (e.g., from `axl-eval --output result.json`) into runtime history. Array form supports multi-run CLI output (`--runs N > 1` writes a JSON array, including for partial batches) — each entry imports as its own history entry; per-run `runGroupId` flows through unchanged so the runs render as a coherent group. Single-object response is `{ id, eval, timestamp }`; array response is `{ imported: [{ id, eval, timestamp }, ...] }`. Per-entry validation pinpoints the offending index when one element of a multi-run array is malformed; import is all-or-nothing. Generates a fresh UUID per entry so repeated imports don't collide. Eval name derivation: `body.eval ?? result.metadata.workflows[0] ?? legacy result.workflow ?? 'imported'`. Imported entries are indistinguishable from native runs in the picker, run detail, comparison, and rescore (rescore requires a matching registered eval name). Blocked in `readOnly`. **This is the only Studio endpoint with potentially large request bodies** — host frameworks must raise their JSON body limit if importing sizeable files
...
- Eval CLI: `axl-eval` binary resolves runtime via three-tier: `--config <path>` → auto-detect `axl.config.*` → bare `new AxlRuntime()`. Supports `--conditions` for monorepo imports. Wraps `executeWorkflow` with `runtime.trackCost()`. Calls `runtime.shutdown()` on completion. Subcommands: `compare` (with `--threshold`, `--fail-on-regression`), `rescore`. Flags: `--runs N` for multi-run, `--output`, `--config`, `--conditions`. Loader-level utilities (`resolveRuntime`/`pickDefault`/`pickExport`/`findConfig`/`needsTsxLoader`/`importModule`/`expandGlob`/`registerConditions`) live in `@axlsdk/axl`'s `cli-internals` so axl-eval and axl-studio share one implementation; `axl-eval`'s own `cli-utils.ts` is a thin re-export plus the eval-specific arg parser. Multi-run mode (`--runs N`) preserves partial batches: when run 3 of 5 fails, runs 1-2 are kept with `metadata.fromPartialBatch: true / batchCompleted / batchAttempted / batchFailure`; the aggregate is computed over the completed runs; the CLI exits non-zero so CI surfaces the failure
- Studio: CLI (`axl-studio`) auto-detects config (`axl.config.mts` → `.ts` → `.mjs` → `.js`), expects `export default runtime`. TypeScript files loaded via tsx's `register()` API (process-wide ESM hook, registered once per process on first TS file) so that chained imports of `.ts` workspace sources are also transformed. The earlier `tsImport()`-per-call approach was namespace-scoped and only transformed the entry file — broke `--conditions development` whenever a monorepo's `development` export pointed at `.ts` paths. `--conditions` flag adds custom import conditions via resolve hook (e.g., `--conditions development` for monorepo source exports). Note: `--conditions` is ESM-only — transitive CJS `require()` chains bypass the hook
