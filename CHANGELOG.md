...
### Added
- **Eval: an item's scorers now run concurrently.** Previously each item's scorers ran serially (`for (const s of scorers) await s.score(...)`), so raising `concurrency` parallelized *across* items but never the judge phase *within* an item — the dominant cost for evals with many `llmScorer` judges. A new `scorerConcurrency?: number` on `EvalConfig` (and `RescoreOptions`) bounds the per-item scorer fan-out; **default 5**, matching the item-level `concurrency` default. Worst-case concurrent scorer calls is therefore `concurrency × scorerConcurrency` (≤25 at defaults), softened by the provider's 429/503/529 backoff. Cost, per-scorer timing, and `scores`/`scoreDetails`/`scorerErrors` ordering are all preserved deterministically (results are folded back in scorer-array order regardless of completion order). The scoring inner loop is now a single shared `scoreItem()` helper used by both `runEval` and `rescore` (one source of truth for determinism + cancellation + cost), and a shared `mapWithConcurrency` worker-pool helper backs both the item loop and the scorer loop. **See _Changed_ for the behavior-change note.**
