...
- Ask-level (`AgentConfig.maxContext`) summaries no longer overwrite durable session summaries or skip messages between a cached summary and a moving tail. Each agent's summary boundary is kept per agent and, in sessions, persisted as session metadata (`askSummary:<agentName>`, storing a SHA-256 hash of the covered prefix rather than the prefix). A later `send()` reuses the boundary with no summary call while the covered prefix, durable session summary, and summary model are unchanged and the full tail fits; agents never share a boundary, and `session.fork()` regenerates rather than copying these keys. `persist: false` sessions write no such records, and a failed cache write is reported as a `log` warning without failing the ask. Boundary reuse across two executions was live-verified on Opus 5.5 with kept post-boundary thinking and no reset.
- Known client-side trims and summaries remove Anthropic thinking signed to the old prefix from retained requests while preserving text, tool calls, and other provider metadata. A `maxContext` removal is reported before the ask's first model turn as a `provider_diagnostic` `reasoning_context_reset` with the new reason `client_prefix_rewrite` and the exact removed count; session-level trimming runs outside an execution and stays unreported. A targeted live Opus 5.5 case accepted the compacted historical tool exchange; exact replay across sessions remains unverified.
- Session retention summaries and `maxContext` summaries share one summary prompt and provider call. `AxlRuntime.summarizeMessages()` accepts an optional `{ previousSummary }` argument instead of a leading system message.
- GPT-6 (`gpt-6-astra`, `gpt-6-sol`, `gpt-6-luna`) drops a portable `temperature` on both `openai:` and `openai-responses:` while reasoning is active, matching GPT-5.x, instead of throwing. Sol and Luna at effective `effort: 'none'` still send it. Raw `providerOptions` that set `temperature`, `top_p`, `top_logprobs`, `logprobs`, or the `message.output_text.logprobs` include under active reasoning are still rejected before dispatch. Live-verified on all three IDs and both endpoints (`docs/verification/frontier-owner-remediation-2026-09-26.md`). OpenAI Chat Completions and Responses now resolve every model-specific rule (effort levels and clamps, default effort, sampling restriction, Chat tool policy, final-request validation) from one model descriptor table, with no wire change for other models.
- Anthropic derives the thinking-binding beta requirement and final-body `block_binding` validation from an explicit per-model capability flag instead of a separate hard-coded model set. Per Anthropic's preserved-thinking documentation, only Claude Opus 5.5 and Claude Fable 5.1 currently carry it.
...
- Generated `maxContext` summaries now emit a paired, ask-scoped `agent_call_start` / `agent_call_end` with `data.purpose: 'summary'`, `turn: 0`, and the summary model URI, so they never collide with the ask's first tool-loop turn. Their known cost joins the ask trace and budget once; unknown-price summaries mark totals as lower bounds. Cached summary reuse incurs no new charge.
...
- `ctx.verify` and `ctx.ask`'s `validate` no longer recover from cancellation. Once the scope they run in is aborted (caller signal, `hard_stop` budget, race or quorum branch), `verify` stops retrying and never returns `fallback`, and the thrown error propagates as-is. A `hard_stop` budget around `verify` now reports `{ value: null, budgetExceeded: true }` instead of the fallback.
- **Behavior change: `ctx.spawn` and `ctx.map` can now reject in their default (non-quorum) mode.** An `AdmissionDeniedError`, or a failure after the enclosing scope was aborted, rejects the call with that error (and, with `quorum`, cancels the remaining work) instead of being folded into an `{ ok: false }` result or reported as `QuorumNotMet`. Callers that relied on these never rejecting should catch around them. `ctx.race` likewise rejects with the outer abort instead of a generic `All race branches were aborted` error. Ordinary branch failures are still folded into results, and branches cancelled by a primitive's own quorum or race winner are still ignored.
- `ctx.budget` no longer reports an outer cancellation or an admission denial as `budgetExceeded`. With `hard_stop`, a workflow/caller abort inside the block returned `{ value: null, budgetExceeded: true }` and the workflow carried on; an `AdmissionDeniedError` thrown after the budget was exceeded was swallowed the same way. Both now reject the block with the original error; the budget's own stop still returns `budgetExceeded: true`.
- `ctx.verify` emitted two `verify` trace events when `validate` rejected the final attempt and no `fallback` was set; it now emits one.
